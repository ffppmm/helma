...
public class ImageObject {
	...
	public static void init(Scriptable scope) {
		...
		ScriptableObject.defineProperty(scope, "Image", ctor,
				ScriptableObject.DONTENUM);
		ctor.put("getInfo", ctor, new GetInfo());
	}

	public static Object imageCtor(Context cx, Object[] args, Function ctorObj,
			boolean inNewExpr) {
		...
		try {
			...
			if (args.length == 1) {
				...
				if (arg instanceof byte[]) {
					...
					// the string could either be a url or a local filename,
					// let's try both:
					...
					img = generator.createImage(((FileObject) arg).getFile()
							.getAbsolutePath());
					...
					if (image != null) {
						img = new ImageWrapper(image, image.getWidth(null),
								image.getHeight(null), generator);
					}
				}
				...
				if (args[0] instanceof Number && args[1] instanceof Number) {
					...
					// create a new image from an existing one and an image
					// filter
					...
					if (wrapper instanceof ImageWrapper) {
						...
						if (filter instanceof ImageFilter)
							img = generator.createImage((ImageWrapper) wrapper,
									(ImageFilter) filter);
		...
		if (img == null) {
			...
			case 0:
				throw new RuntimeException(
						"Error creating Image: Called without arguments");
			case 1:
				throw new RuntimeException("Error creating Image from "
						+ args[0]);
			case 2:
				throw new RuntimeException("Error creating Image from "
						+ args[0] + ", " + args[1]);
			default:
				throw new RuntimeException(
						"Error creating Image: Wrong number of arguments");
			}
		}
