...
public class XmlObject {
	...
	public boolean write(INode node, String file, boolean dbmode)
		...
		if (node == null) {
			throw new RuntimeException(
					"First argument in Xml.write() is not an hopobject");
		}

		if (file == null) {
			throw new RuntimeException(
					"Second argument file name must not be null");
		}
		...
		core.getApplication().logEvent(
				"wrote xml to " + finalFile.getAbsolutePath());
	...
	/**
	 * Reads an XML document from a file and reads it into the HopObject
	 * argument
	 * 
	 * @param file
	 *            the file name
	...
	public Object read(String file, INode node) throws RuntimeException {
		...
		if (node == null) {
			// make sure we have a node, even if 2nd arg doesn't exist or is not
			// a node
			node = new PersistentNode(null, null, core.getApplication()
	...
	/**
	 * Reads an XML document from an XML literal and creates a HopObject out of
	 * it
	 * 
	 * @param str
	 *            the XML string
	...
	/**
	 * Retrieves an XML document from a given URL and transforms it into a
	 * HopObject
	 * 
	 * @param url
	...
	public Object get(String url, String conversionRules) {
		if (url == null) {
			throw new RuntimeException(
					"Xml.get() needs a location as an argument");
		}
		...
		try {
			...
			INode node = new PersistentNode(null, null, core.getApplication()
					.getWrappedNodeManager());
	...
	/**
	 * Transforms a XML literal into a HopObject according to the rules
	 * specified in the file defined by conversionRules
	 * 
	 * @param str
	 *            an XML literal
	 * @param conversionRules
	...
	public Object getFromString(String str, String conversionRules) {
		if (str == null) {
			throw new RuntimeException(
					"Xml.getFromString() needs an XML string as parameter");
		}
