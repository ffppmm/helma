...
/**
 * A JavaScript wrapper around a JavaMail message class to send mail via SMTP
 * from Helma
 */
public class MailObject extends ScriptableObject implements Serializable {
	...
	/**
	 * Get the cached JavaMail session. This is similar to
	 * Session.getDefaultSession(), except that we check if the properties have
	 * changed.
	 */
	protected Session getSession() {
		if (props == null) {
			throw new NullPointerException(
					"getSession() called on non-prototype MailObject");
		}
		...
		if (session == null || hostChanged) {
			...
			// create properties and for the Session. Only set mail host if it
			// is
	...
	public static void init(Scriptable scope, Properties props) {
		...
		String[] mailFuncs = { "addBCC", "addCC", "addPart", "addText",
				"addTo", "send", "setFrom", "setSubject", "setText", "setTo",
	...
	/**
	 * Returns the MIME multipart message subtype. The default value is "mixed"
	 * for messages of type multipart/mixed. A common value is "alternative" for
	...
	/**
	 * Sets the MIME multipart message subtype. The default value is "mixed" for
	 * messages of type multipart/mixed. A common value is "alternative" for the
		...
		try {
			if (obj == null || obj == Undefined.instance) {
				throw new IOException(
						"mail.addPart called with wrong number of arguments.");
			}
	 ...
	 * 
	 * @param addstr
	 *            the email address to set in the From header
	 * @param name
	 ...
	 * 
	 * @param addstr
	 *            the email address to set in the To header
	 * @param name
	 *            the name this address belongs to
	 */
	 ...
	 * 
	 * @param addstr
	 *            the email address to set in the CC header
	 * @param name
	 *            the name this address belongs to
	 */
	 ...
	 * 
	 * @param addstr
	 *            the email address to set in the BCC header
	 * @param name
	 *            the name this address belongs to
	 */
	 ...
	 *            the name this address belongs to
	 * @param type
	 *            the type of the recipient such as To, CC, BCC
	 * 
	 * @throws Exception
	 *             ...
	 ...
	 *             ...
	 */
	private void addRecipient(String addstr, Object name,
		...
		if (name != null && name != Undefined.instance) {
			address = new InternetAddress(addstr, MimeUtility.encodeWord(name
					.toString()));
		} else {
			address = new InternetAddress(addstr);
		}
