...
public class Profiler implements Debugger {
	...
	/**
	 * Implementws handleCompilationDone in interface
	...
	static String getFunctionName(DebuggableScript script) {
		if (script.isFunction()) {
			StringBuffer b = new StringBuffer(script.getSourceName())
					.append(" #");
	...
	public String getResult() {
		ProfilerFrame[] f = (ProfilerFrame[]) frames.values().toArray(
				new ProfilerFrame[0]);
		Arrays.sort(f, new Comparator<Object>() {
			public int compare(Object o1, Object o2) {
				return ((ProfilerFrame) o2).runtime
						- ((ProfilerFrame) o1).runtime;
			}
		});
		...
		for (int i = 0; i < length; i++) {
			...
			if (i < length - 1) {
				prefixLength = Math.min(prefixLength, StringUtils
						.getCommonPrefix(f[i].name, f[i + 1].name).length());
			}
		}
	...
	class ProfilerFrame implements DebugFrame {
		...
		public void onExceptionThrown(Context cx, Throwable ex) {
			// TODO: figure out if this is called in addition or in place of to
			// onExit
		}
		...
		public String renderLine(int prefixLength) {
			...
			Object[] args = new Object[] { Integer.valueOf((int) millis),
					Integer.valueOf(Math.round(millis / invocations)),
					Integer.valueOf(invocations), name.substring(prefixLength) };
