...
public class HopObject extends ScriptableObject implements Wrapper,
	 ...
	 */
	protected HopObject(String className, RhinoCore core, INode node,
			Scriptable proto) {
		this.className = className;
		this.core = core;
	 ...
	 *            the RhinoCore
	 * @param handle
	 *            the handle for the wrapped node
	 * @param proto
	 *            the object's prototype
	 */
	protected HopObject(String className, RhinoCore core, NodeHandle handle,
			Scriptable proto) {
		this.className = className;
		this.core = core;
	...
	public static HopObject init(RhinoCore core) {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if (methodName.startsWith("jsFunction_")) {
				...
				proto.defineProperty(methodName, null, methods[i], null,
						attributes);
			}
		}
		return proto;
	...
	/**
	 * Overwritten to not define constructor property as constant - we need to
	 * have the constructor property resettable in Helma.
	 * 
	 * @param propertyName
	 *            the property name
	 * @param value
	...
	/**
	 * Return a primitive representation for this object. FIXME: We always
	...
	public Object jsFunction_getResource(String resourceName) {
		...
		while (prototype != null) {
			...
			for (int i = resources.length - 1; i >= 0; i--) {
				...
				if (resource.exists()
						&& resource.getShortName().equals(resourceName))
	...
	public Object jsFunction_href(Object action, Object params)
		...
		Properties queryParams = params instanceof Scriptable ? core
				.scriptableToProperties((Scriptable) params) : null;
		INode node = getNode();
	 ...
	 * 
	 * @param id
	 *            The name/id or index, depending if the argument is a String or
	 *            Number.
	 * 
	 * @return ...
	 */
	...
	public Object jsFunction_getById(Object id) {
		...
		INode node = getNode();
		String idString = (id instanceof Double) ? Long.toString(((Double) id)
				.longValue()) : id.toString();
	...
	public boolean jsFunction_set(Object id, Object value) {
		if (id == Undefined.instance || value == Undefined.instance) {
			throw new EvaluatorException(
					"HopObject.set() called with wrong number of arguments");
		}
		if (proxy == null) {
			return false;
		}
		...
		if (id instanceof Number) {
			if (!(value instanceof HopObject)) {
				throw new EvaluatorException(
						"Can only set HopObjects as child objects in HopObject.set()");
			}
	...
	private void prefetchChildren(int start, int length) {
		if (proxy != null) {
			...
			if (node instanceof PersistentNode) {
				...
				if (n.getState() != INode.TRANSIENT
						&& n.getState() != INode.NEW) {
	...
	/**
	 * Return the full list of child objects in a JavaScript Array. This is
	 * called by jsFunction_list() if called with no arguments.
	 * 
	...
	public Scriptable jsFunction_list(Object startArg, Object lengthArg) {
		...
		if (start < 0 || length < 0) {
			throw new EvaluatorException(
					"Arguments must not be negative in HopObject.list(start, length)");
		}
	...
	public boolean jsFunction_remove(Object arg) {
		...
		if (arg != Undefined.instance) {
			System.err
					.println(" *************  WARNING  *************************");
			System.err
					.println(" The version of HopObject.remove(child) you were ");
			System.err
					.println(" trying to use has been deprecated. Please use ");
			System.err.println("      hopobj.removeChild(child)");
			System.err
					.println(" to remove a child object from a collection without");
			System.err.println(" deleting it, or ");
			System.err.println("      hopobj.remove()");
			System.err
					.println(" without argument to delete the object itself.");
			System.err
					.println(" *************************************************");
			throw new RuntimeException(
					"Caught deprecated usage of HopObject.remove(child)");
		}
	...
	/**
	 * Remove a child node from this node's collection without deleting it from
	 * the database.
	 */
	 ...
	 * 
	 * @return the ID of the newly persisted HopObject or null if operation
	 *         failed
	 */
	...
	/**
	 * Check whether the wrapped Node is persistent. This also returns true if
	 * the Node is being inserted in the database, or it has been in database
	 * and is in the proces of being deleted.
	 * 
	...
	/**
	 * Check whether the wrapped Node is transient. This also returns false if
	 * the Node is being inserted in the database, or it has been in database
	 * and is in the proces of being deleted.
	 * 
	...
	/**
	 * Check if node is contained in the subnode collection. Return its index
	 * position if it is, and -1 otherwise.
	 */
		...
		if (proxy == null) {
			...
			if (isRecording) {
				...
				if (value instanceof Function) {
					// reset function's parent scope, needed because of the way
					// we compile
			...
			if ((value == null) || (value == Undefined.instance)) {
				...
				if ("Date".equals(className)) {
					node.setDate(name,
							new Date((long) ScriptRuntime.toNumber(s)));
	...
	/**
	 * Retrieve a property only from the node itself, not the underlying
	 * prototype object. This is called directly when we call get(x) on a JS
	 * HopObject, since we don't want to return the prototype functions in that
	 * case.
	 */
	...
	/**
	 * Return all property names of this object. This method is used by
	 * debugger.
	 * 
	 * @return array containing the names of all properties defined in this
	 *         object
	 */
	...
	/**
	 * Return all "ordinary" property ids of this object. This "hides" the
	 * prototype methods.
	 * 
	...
	/**
	 * Custom <tt>==</tt> operator. Must return
	 * {@link org.mozilla.javascript.Scriptable#NOT_FOUND} if this object does
	 * not have custom equality operator for the given value,
			...
			if (node == null || node.getState() == INode.INVALID) {
				if (handle != null) {
					...
					if (node != null) {
						...
						// the actual prototype name may vary from the node
						// handle's prototype name
						if (className == null || !className.equals(protoname)) {
							Scriptable proto = core
									.getValidPrototype(protoname);
							if (proto == null) {
								protoname = "HopObject";
				...
				if (node == null || node.getState() == INode.INVALID) {
					...
					// Replace with empty transient node to avoid throwing an
					// exception.
