 ...
 * Example:
 * 
 * var y = { __get__ : function (name) { ... } __has__ : function (name) { ... }
 * __put__ : function (name, value) {...} __delete__ : function (name) { ... }
 * __getIds__ : function () { ... } };
 * 
 * var x = new JSAdapter(y);
 * 
 * x.i; // calls y.__get__ i in x; // calls y.__has__ x.p = 10; // calls
 * y.__put__ delete x.p; // calls y.__delete__ for (i in x) { print(i); } //
 * calls y.__getIds__
 * 
 ...
 * adaptee. Use cases include 'smart' properties, property access
 * tracing/debugging, encaptulation with easy client access - in short
 * JavaScript becomes more "Self" like.
 * 
 * Note that Rhino already supports special properties like __proto__ (to set,
 * get prototype), __parent__ (to set, get parent scope). We follow the same
 * double underscore nameing convention here. Similarly the name JSAdapter is
 * derived from JavaAdapter -- which is a facility to extend, implement Java
 * classes/interfaces by JavaScript.
 * 
 * @version 1.0
...
public final class JSAdapter implements Scriptable, Function {
	...
	// TODO: check these strange Class things
	...
	public Scriptable construct(Context cx, Scriptable scope, Object[] args)
		...
		if (isPrototype) {
			...
			if (tmp instanceof Function) {
				...
				throw Context
						.reportRuntimeError("TypeError: not a constructor");
			}
		}
	}
