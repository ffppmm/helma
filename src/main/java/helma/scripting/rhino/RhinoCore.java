...
/**
 * This is the implementation of ScriptingEnvironment for the Mozilla Rhino
...
public final class RhinoCore implements ScopeProvider {
	...
	protected synchronized void initialize() {
		...
		try {
			...
			// add some convenience functions to string, date and number
			// prototypes
			Scriptable stringProto = ScriptableObject.getClassPrototype(global,
					"String");
			stringProto.put("trim", stringProto, new StringTrim());

			Scriptable dateProto = ScriptableObject.getClassPrototype(global,
					"Date");
			dateProto.put("format", dateProto, new DateFormat());

			Scriptable numberProto = ScriptableObject.getClassPrototype(global,
					"Number");
	 ...
	 * 
	 * @param type
	 *            the info, containing the object proto, last update time and
	...
	protected void setParentPrototype(Prototype prototype, TypeInfo type) {
		...
		if (!"global".equals(lowerCaseName)
			...
			if (parent != null) {
				// see if parent prototype is already registered. if not,
				// register it
	 ...
	 * here is to check for update those prototypes which already have been
	 * compiled before. Others will be updated/compiled on demand.
	 */
	public synchronized void updatePrototypes() throws IOException {
		...
		for (Iterator<?> i = protos.iterator(); i.hasNext();) {
			...
			if (type == null) {
				// a prototype we don't know anything about yet. Init local
				// update info.
				initPrototype(proto);
			} else if (type.lastUpdate > -1) {
				// only need to update prototype if it has already been
				// initialized.
	 ...
	 * 
	 * @param type
	 *            the type info to check
	 * @param checked
	 *            a set of prototypes that have already been checked
	 */
	...
	/**
	 * A version of getPrototype() that retrieves a prototype and checks if it
	 * is valid, i.e. there were no errors when compiling it. If invalid, a
	 * ScriptingException is thrown.
	 */
	...
	/**
	 * Get the object prototype for a prototype name and initialize/update it if
	 * necessary. The policy here is to update the prototype only if it hasn't
	 * been updated before, otherwise we assume it already was updated by
	 * updatePrototypes(), which is called for each request.
	 */
	 ...
	 * @return an array containing all compiled properties of the given
	 *         prototype
	 */
	...
	/**
	 * Private helper function that retrieves a prototype's TypeInfo and creates
	 * it if not yet created. This is used by getPrototype() and
	 * getValidPrototype().
	 */
	private TypeInfo getPrototypeInfo(String protoName) {
		...
		// otherwise, it has already been evaluated for this request by
		// updatePrototypes(),
	...
	/**
	 * Check if an object has a function property (public method if it is a java
	...
	/**
	 * Get a Script wrapper for any given object. If the object implements the
	 * IPathElement interface, the getPrototype method will be used to retrieve
	 * the name of the prototype to use. Otherwise, a
	 * Java-Class-to-Script-Prototype mapping is consulted.
	 */
	...
	protected String postProcessHref(Object obj, String protoName, String href)
		...
		// check if the app.properties specify a href-function to post-process
		// the
		// basic href.
		...
		if (hrefFunction != null) {
			...
			while (handler != null) {
				if (hasFunction(proto, hrefFunction)) {
					...
					// get the currently active rhino engine and invoke the
					// function
					...
					try {
						...
						throw new EvaluatorException("Error in hrefFunction: "
								+ x);
					}

					if (result == null) {
						throw new EvaluatorException("hrefFunction "
								+ hrefFunction + " returned null");
					}
		...
		if (hrefSkin != null) {
			...
			while (handler != null) {
				...
				if (skin != null) {
					Scriptable param = Context.getCurrentContext().newObject(
							global);
					param.put("path", param, href);
					href = skin.renderAsString(eng.getRequestEvaluator(),
							handler, param).trim();
					break;
				}
	...
	/**
	 * Add a code resource to a given prototype by immediately compiling and
	 * evaluating it.
	 * 
	 * @param typename
	 *            the type this resource belongs to
	 * @param code
	 *            a code resource
	 */
		...
		try {
			...
			if (sourceName.endsWith(".js")) {
				reader = encoding == null ? new InputStreamReader(
						code.getInputStream()) : new InputStreamReader(
						code.getInputStream(), encoding);
				...
				reader = new StringReader(HacHspConverter.convertHsp(code,
						encoding));
				cx.evaluateReader(op, reader, sourceName, 0, null);
				reader = new StringReader(HacHspConverter.convertHspAsString(
						code, encoding));
				cx.evaluateReader(op, reader, sourceName, 0, null);
			}
	...
	class TypeInfo {
		...
		// timestamp of last update. This is -1 so even an empty prototype
		// directory
		// (with lastUpdate == 0) gets evaluated at least once, which is
		// necessary
		...
		// a set of property keys that were present before first script
		// compilation
		...
		/**
		 * If prototype implements PropertyRecorder tell it to start registering
		 * property puts.
		 */
		public void prepareCompilation() {
			...
			// mark this type as updated so injectCodeResource() knows it's
			// initialized
		...
		public void commitCompilation() {
			...
			if (objProto instanceof PropertyRecorder) {
				...
				if (changedProperties != null) {
					...
					// remove all renewed properties from the previously
					// compiled
					// property names so we can remove those properties that
					// were not
					// renewed in this compilation
					compiledProperties.removeAll(changedProperties);

					boolean isGlobal = "global".equals(frameworkProto
							.getLowerCaseName());
					...
					while (it.hasNext()) {
						...
						if (isGlobal
								&& (prototypes.containsKey(key.toLowerCase()) || "JavaPackage"
										.equals(key))) {
						...
						try {
							...
							app.logEvent("Error unsetting property " + key
									+ " on " + frameworkProto.getName());
						}
					}
	...
	class WrapMaker extends WrapFactory {
		...
		public Object wrap(Context cx, Scriptable scope, Object obj,
				Class<?> staticType) {
			// taking a shortcut here on things normally defined by
			// setJavaPrimitivesWrap()
				...
				if (args.length > 1 && args[1] instanceof NativeJavaObject) {
					...
					if (locale instanceof Locale) {
						...
						throw new IllegalArgumentException(
								"Second argument to Date.format() not a java.util.Locale: "
										+ locale.getClass());
					}
				} else {
					df = new SimpleDateFormat(args[0].toString());
	...
	class HelmaContextFactory extends ContextFactory {
		...
		final boolean strictVars = "true".equalsIgnoreCase(app
				.getProperty("strictVars"));
		...
		protected void onContextCreated(Context cx) {
			...
			if (Context.isValidLanguageVersion(languageVersion)) {
				...
				app.logError("Unsupported rhino.languageVersion: "
						+ languageVersion);
			}
		...
		/**
		 * Implementation of
		 * {@link Context#observeInstructionCount(int instructionCount)}. This
		 * can be used to customize {@link Context} without introducing
