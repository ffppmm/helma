...
public class HopObjectCtor extends FunctionObject {
	...
	static {
		try {
			hopObjCtor = HopObjectCtor.class.getMethod("jsConstructor",
					new Class[] { Context.class, Object[].class,
							Function.class, Boolean.TYPE });
		} catch (NoSuchMethodException e) {
			throw new RuntimeException(
					"Error getting HopObjectCtor.jsConstructor()");
		}
	}
	...
	/**
	 * Create and install a HopObject constructor. Part of this is copied from
	 * o.m.j.FunctionObject.addAsConstructor().
	 * 
	 * @param prototype
	 */
	...
	public static Object jsConstructor(Context cx, Object[] args,
			Function ctorObj, boolean inNewExpr) throws JavaScriptException {
		...
		public Object call(Context cx, Scriptable scope, Scriptable thisObj,
			...
			if (args.length < 1 || args.length > 2)
				throw new IllegalArgumentException(
						"Wrong number of arguments in getById()");
			// If second argument is provided, use it as type name.
			// Otherwise, use our own type name.
			String type = args.length == 1 ? HopObjectCtor.this
					.getFunctionName() : Context.toString(args[1]);
	...
	class HopCollection extends BaseFunction {
		...
		public Object call(Context cx, Scriptable scope, Scriptable thisObj,
			...
			if (args.length != 1) {
				throw new IllegalArgumentException(
						"Wrong number of arguments in definePrototype()");
			}
			if (!(args[0] instanceof Scriptable)) {
				throw new IllegalArgumentException(
						"Second argument to HopObject.definePrototype() must be Object");
			}
			...
			if (!childmapping.containsKey("collection")) {
				// if contained type isn't defined explicitly limit collection
				// to our own type
				childmapping.put("collection",
			...
			PersistentNode node = new PersistentNode("HopQuery",
					Long.toString(collectionId++), null, nmgr);
