...
public class ApplicationManager implements XmlRpcHandler {
	 ...
	 * 
	 * @param props
	 *            the properties defining the running apps
	 * @param server
	 *            the server instance
	 */
	...
	/**
	 * Called regularely check applications property file to create and start
	 * new applications.
	 */
	protected void checkForChanges() {
		if (props.lastModified() > lastModified
				&& server.getApplicationsOption() == null) {
			try {
				...
				for (Enumeration<AppDescriptor> e = descriptors.elements(); e
						.hasMoreElements();) {
					AppDescriptor appDesc = (AppDescriptor) e.nextElement();

					// check if application has been removed and should be
					// stopped
	 ...
	 */
	public void stopAll() {
		for (Enumeration<AppDescriptor> en = descriptors.elements(); en
				.hasMoreElements();) {
			try {
	...
	public Object execute(String method, Vector params) throws Exception {
		...
		if ((dot == 0) || (dot == (method.length() - 1))) {
			throw new Exception("\"" + method
					+ "\" is not a valid XML-RPC method name");
		}
		...
		if (app == null) {
			...
			// use the original method name, the handler is resolved within the
			// app.
			method2 = method;
		}

		if (app == null) {
			throw new Exception("Handler \"" + handler + "\" not found for "
					+ method);
		}
	...
	class AppDescriptor {
		...
		AppDescriptor(String name) {
			...
			staticMountpoint = getPathPattern(conf.getProperty(
					"staticMountpoint", joinMountpoint(mountpoint, "static")));
			staticIndex = "true".equalsIgnoreCase(conf
					.getProperty("staticIndex"));
			...
			if (appDir != null) {
				...
				repositoryList.add(new FileRepository(new File(server
						.getAppsHome(), appName)));
			}
		...
		void start() {
			...
			try {
				...
				app = new Application(appName, server, repositories, appDir,
						dbDir);
				...
				// the application is started later in the register method, when
				// it's bound
				app.init(ignoreDirs);
		...
		void bind() {
			try {
				getLogger().info(
						"Binding application " + appName + " :: "
								+ app.hashCode() + " :: " + this.hashCode());
				...
				if (jetty != null) {
					...
					if (staticDir != null) {
						...
						getLogger().info(
								"Mounting static at " + staticMountpoint);
					...
					@SuppressWarnings("rawtypes")
					Class servletClass = servletClassName == null ? EmbeddedServletClient.class
					...
					if (sessionCookieName != null) {
						holder.setInitParameter("sessionCookieName",
								sessionCookieName);
					}

					if (protectedSessionCookie != null) {
						holder.setInitParameter("protectedSessionCookie",
					...
					if (uploadSoftfail != null) {
						holder.setInitParameter("uploadSoftfail",
								uploadSoftfail);
					}
