...
public class JettyServer {
	...
	// the AJP13 Listener, used for connecting from external webserver to
	// servlet via JK
	protected Ajp13SocketConnector ajp13;

	public static JettyServer init(Server server, ServerConfig config)
			throws IOException {
		File configFile = config.getConfigFile();
		if (configFile != null && configFile.exists()) {
			return new JettyServer(configFile.toURI().toURL());
		} else if (config.hasWebsrvPort() || config.hasAjp13Port()) {
			return new JettyServer(config.getWebsrvPort(),
					config.getAjp13Port(), server);
		}
		return null;
	}
	...
	private JettyServer(InetSocketAddress webPort, InetSocketAddress ajpPort,
		...
		if (ajpPort != null) {
			...
			server.getLogger().info(
					"Starting AJP13-Listener on port " + (ajpPort));
		}
		openListeners();
	}
	...
	private void openListeners() throws IOException {
		// opening the listener here allows us to run on priviledged port 80
		// under jsvc
