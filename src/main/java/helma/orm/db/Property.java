...
/**
 * A property implementation for Nodes stored inside a database. Basically the
 * same as for transient nodes, with a few hooks added.
 */
public final class Property implements IProperty, Serializable, Cloneable,
	 ...
	 * 
	 * 
	 * @return the property's type as defined in
	 *         helma.objectmodel.IProperty.java
	 */
	public int getType() {
		return type;
	}
	...
	public void convertToNodeReference(Relation rel) {
		if ((value != null) && !(value instanceof NodeHandle)) {
			if (rel.usesPrimaryKey()) {
				value = new NodeHandle(new DbKey(rel.otherType,
						value.toString()));
			} else {
				value = new NodeHandle(new MultiKey(rel.otherType,
						rel.getKeyParts(node)));
			}
		}
	 ...
	 * 
	 *      The following cases throw a ClassCastException - Properties of a
	 *      different type - Properties of boolean or node type
	 */
	public int compareTo(Object obj) {
		...
		if (type == NODE || ptype == NODE || type == BOOLEAN
				|| ptype == BOOLEAN) {
			throw new ClassCastException("uncomparable values " + this + "("
					+ type + ") : " + p + "(" + ptype + ")");
		}
		...
		if (type != ptype) {
			...
			if ((type == FLOAT && ptype == INTEGER)
					|| (type == INTEGER && ptype == FLOAT))
				return Double.compare(((Number) value).doubleValue(),
						((Number) pvalue).doubleValue());
			throw new ClassCastException("uncomparable values " + this + "("
					+ type + ") : " + p + "(" + ptype + ")");

		}
		if (!(value instanceof Comparable)) {
			throw new ClassCastException("uncomparable value " + value + "("
					+ value.getClass() + ")");
		}
		// System.err.println("COMPARING: " + value.getClass() + " TO " +
		// pvalue.getClass());
		// FIXME: Type?
