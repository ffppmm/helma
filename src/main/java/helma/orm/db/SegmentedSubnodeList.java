...
public class SegmentedSubnodeList extends SubnodeList {
	...
	public synchronized Object[] toArray() {
		...
		node.getNodeManager().logEvent(
				"Warning: toArray() called on large segmented collection: "
						+ node);
		for (int i = 0; i < segments.length; i++) {
			loadSegment(i, false);
		}
	...
	private List<NodeHandle> loadSegment(int seg, boolean deep) {
		...
		if (segment != null && !segment.loaded) {
			...
			List<NodeHandle> seglist = deep ? node.getNodeManager().getNodes(
					node, rel) : node.getNodeManager().getNodeIDs(node, rel);
			int actualSize = seglist.size();
			if (actualSize != expectedSize) {
				node.getNodeManager()
						.logEvent(
								"Inconsistent segment size in " + node + ": "
										+ segment);
			}
	...
	protected synchronized void update() {
		...
		if (lastChange != lastSubnodeFetch) {
			...
			subnodeCount = node.getNodeManager().countNodes(node,
					getSubnodeRelation());
			if (subnodeCount > SEGLENGTH) {
				...
				for (int s = 0; s < nsegments; s++) {
					int length = (s == nsegments - 1 && remainder > 0) ? remainder
							: SEGLENGTH;
