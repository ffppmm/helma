...
/**
 * This class describes a releational data source (URL, driver, user and
 * password).
 */
public class DbSource {
	...
	public synchronized Connection getConnection()
		...
		if (con == null || con.isClosed() || fileUpdated) {
			...
			// false here and make commit/rollback invocations in Transactor
			// methods;
	 ...
	 * 
	 * @param newProps
	 *            the new properties to use for this db source
	 * @return the old properties
	 * @throws ClassNotFoundException
	 *             if jdbc driver class couldn't be found
	 */
	public synchronized ResourceProperties switchProperties(
			ResourceProperties newProps) throws ClassNotFoundException {
	...
	private synchronized void init() throws ClassNotFoundException {
		lastRead = (defaultProps == null) ? props.lastModified() : Math.max(
				props.lastModified(), defaultProps.lastModified());
		...
		if (url == null) {
			throw new NullPointerException(name
					+ ".url is not defined in db.properties");
		}
		if (driver == null) {
			throw new NullPointerException(name
					+ ".driver class not defined in db.properties");
		}
		...
		for (Enumeration<?> e = subProps.keys(); e.hasMoreElements();) {
			...
			if ("url".equalsIgnoreCase(key) || "driver".equalsIgnoreCase(key)
	...
	public boolean equals(Object obj) {
		return obj instanceof DbSource
				&& subProps.equals(((DbSource) obj).subProps);
	}
