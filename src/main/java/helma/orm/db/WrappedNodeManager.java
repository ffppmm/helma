...
import helma.orm.ObjectNotFoundException;

import java.util.List;
import java.util.Vector;

/**
 * A wrapper around NodeManager that catches most Exceptions, or rethrows them
 * as RuntimeExceptions. The idea behind this is that we don't care a lot about
 * Exception classes, since Hop programming is done in JavaScript which doesn't
 * know about them (except for the exception message).
 */
public final class WrappedNodeManager {
	...
	public INode getNode(PersistentNode home, String id, Relation rel) {
		...
		try {
			...
			abortLocalTransaction(tx);
			nmgr.app.logError("Error retrieving Node \"" + id + "\" from "
					+ home, x);
			throw new RuntimeException("Error retrieving Node", x);
		}
	}
	...
	/**
	 * Get the list of nodes contained in the collection of the given Node
	...
	/**
	 * Get a list of IDs of nodes contained in the given Node's collection
	 * specified by the given Relation.
	 * 
	 * @param home
	 * @param rel
	 ...
	 */
	/*
	 * public int updateSubnodeList (Node home, Relation rel) { try { return
	 * nmgr.updateSubnodeList(home, rel); } catch (Exception x) { throw new
	 * RuntimeException("Error retrieving NodeIDs", x); } }
	 */

	/**
	 * Count the nodes contained in the given Node's collection specified by the
	 * given Relation.
	 * 
	 * @param home
	 * @param rel
	...
	/**
	 * Get a list of property names from the given node. TODO: this retrieves
	 * access names of child nodes, not property names
	 * 
	 * @param home
	 * @param rel
	 * @return
	 ...
	 * 
	 * @param map
	 *            the DbMapping to generate an id for
