 ...
 * relational database table. This can be either a scalar property (string,
 * date, number etc.) or a reference to one or more other objects.
 */
public final class Relation {
	...
	// a 1-to-1 relation, i.e. a field in the table is a foreign key to another
	// object
	public final static int REFERENCE = 1;

	// a 1-to-many relation, a field in another table points to objects of this
	// type
	...
	// the DbMapping of the prototype we link to, unless this is a "primitive"
	// (non-object) relation
	DbMapping otherType;
	...
	// if this relation defines a virtual node, we need to provide a DbMapping
	...
	/**
	 * This constructor makes a copy of an existing relation. Not all fields are
	 * copied, just those which are needed in groupby- and virtual nodes defined
	 * by this relation. use {@link Relation#getClone()} to get a full copy of
	 * this relation.
	 */
	protected Relation(Relation rel) {
		// Note: prototype, groupby, groupbyPrototype and groupbyOrder aren't
		// copied here.
		// these are set by the individual get*Relation() methods as
		// appropriate.
	 ...
	 * 
	 * @param desc
	 *            the top level relation descriptor. For relations defined in a
	 *            type.properties file, this is a string like
	 *            "collection(Type)", but for relations defined from JavaScript,
	 *            it is the top level descriptor object.
	 * @param props
	 *            The subproperties for this relation.
	 */
	 ...
	 * 
	 * @param value
	 *            the value of the top level property mapping
	 * @param config
	 *            the sub-map for this property mapping
	 * @return true if the value describes a valid, non-primitive property
	 *         mapping
	 */
		...
		if (desc == null || "".equals(desc.trim())) {
			...
			if (open > -1 && close > open) {
				...
				if ("collection".equalsIgnoreCase(ref)) {
					...
					throw new RuntimeException("Invalid property Mapping: "
							+ desc);
				}
	...
	protected void parseOptions(Vector<Constraint> cnst, Properties props) {
		...
		if (loading != null) {
			...
			if ("aggressive".equalsIgnoreCase(loading)) {
				...
				System.err.println("Unsupported loadmode property in "
						+ ownType + ": " + loading);
			...
			if (additionalTables.trim().length() == 0) {
				...
				if (dbsource != null) {
					...
					for (int i = 0; i < tables.length; i++) {
						// Skip some join-related keyworks we might encounter
						// here
		...
		if (groupby != null) {
			...
			if (groupbyPrototype != null
					&& groupbyPrototype.trim().length() == 0) {
	...
	private int getIntegerProperty(String name, Properties props,
			int defaultValue) {
		...
		if (value instanceof Number) {
			...
			try {
				...
				ownType.getApplication().logError(
						"Can't parse integer for property " + name
								+ " from value " + value, nfx);
			}
		}
		return defaultValue;
	}
	...
	/**
	 * Return the target type of this relation, or null if this is a primitive
	 * mapping.
	 */
	...
	/**
	 * Returns true if this Relation describes either a primitive value or an
	 * object reference.
	 */
	...
	/**
	 * Returns true if this Relation describes a collection. <b>NOTE:</b> this
	 * will return true both for collection objects (aka virtual nodes) and
	 * direct child object relations, so isVirtual() should be used to identify
	 * relations that define <i>collection properties</i>!
	 */
	...
	/**
	 * Returns true if this Relation describes a complex object reference
	 * property
	 */
	...
	/**
	 * Tell wether the property described by this relation is to be handled as
	 * private, i.e. a change on it should not result in any changed
	 * object/collection relations.
	 */
	...
	/**
	 * Returns true if the object represented by this Relation has to be created
	 * on demand at runtime by the NodeManager. This is true for:
	 * 
	 * - collection (aka virtual) nodes - nodes accessed via accessname - group
	 * nodes - complex reference nodes
	 */
	 ...
	 * object stored in the db, since a virtual collection would lose its its
	 * content after restarts.
	 */
	 ...
	 * 
	 * 
	 * @return true if the foreign key used for this relation is the other
	...
	/**
	 * This is taken from org.apache.tools.ant ProjectHelper.java distributed
	 ...
	 */
	protected void parsePropertyString(String value, Vector<String> fragments,
			Vector<String> propertyRefs) {
		int prev = 0;
		int pos;
	...
	Relation getVirtualSubnodeRelation() {
		if (!virtual) {
			throw new RuntimeException(
					"getVirtualSubnodeRelation called on non-virtual relation");
		}
	...
	Relation getVirtualPropertyRelation() {
		if (!virtual) {
			throw new RuntimeException(
					"getVirtualPropertyRelation called on non-virtual relation");
		}
	...
	Relation getGroupbySubnodeRelation() {
		if (groupby == null) {
			throw new RuntimeException(
					"getGroupbySubnodeRelation called on non-group-by relation");
		}
	...
	Relation getGroupbyPropertyRelation() {
		if (groupby == null) {
			throw new RuntimeException(
					"getGroupbyPropertyRelation called on non-group-by relation");
		}
	...
	/**
	 * Build the second half of an SQL select statement according to this
	 * relation and a local object.
	 */
	public void buildQuery(StringBuffer q, INode home, boolean useOrder,
			boolean isCount) throws SQLException, ClassNotFoundException {
	 ...
	 * relation and a local object.
	 */
	public void buildQuery(StringBuffer q, INode home, DbMapping otherDbm,
			String kstr, boolean useOrder, boolean isCount)
		...
		if (kstr != null && !isComplexReference()) {
			...
			String accessColumn = (accessName == null) ? otherDbm.getIDField()
					: accessName;
		...
		if (maxSize > 0 && kstr == null) {
			if (otherType.isOracle()) {
				...
				if (offset > 0) {
					...
					q.append(") a WHERE ROWNUM <= ").append(offset + maxSize)
							.append(") WHERE rnum > ").append(offset);
				} else {
					q.insert(0, "SELECT /*+ FIRST_ROWS(n) */ " + selectItem
							+ " FROM (");
				...
				if (fragment == null) {
					...
					if (columnName != null) {
						...
						if (value == null) {
							...
							} else if (columnName.equalsIgnoreCase(dbmap
									.getNameField())) {
	 ...
	 *            our home node
	 * @param prefix
	 *            the prefix to use to append to the existing query (e.g.
	 *            " AND ")
	 * 
	 * @throws SQLException
	...
	public void renderConstraints(StringBuffer q, INode home, String prefix)
			throws SQLException, ClassNotFoundException {
		renderConstraints(q, home, home.getNonVirtualParent(), otherType,
				prefix);
	}
	 ...
	 *            our home node
	 * @param nonvirtual
	 *            our non-virtual home nod
	 * @param otherDbm
	 *            the DbMapping of the remote Node
	 * @param prefix
	 *            the prefix to use to append to the existing query (e.g.
	 *            " AND ")
	 * 
	...
	public void renderConstraints(StringBuffer q, INode home, INode nonvirtual,
			DbMapping otherDbm, String prefix) throws SQLException,
	...
	/**
	 * Render the constraints for this relation for use within a left outer join
	 * select statement for the base object.
	 * 
	 * @param select
	 *            the string buffer to write to
	 * @param isOracle
	 *            create Oracle pre-9 style left outer join
	 */
	...
	/**
	 * Check if the child node fullfills the constraints defined by this
	 * relation. FIXME: This always returns false if the relation has a filter
	 * value set, since we can't determine if the filter constraints are met
	 * without querying the database.
	 * 
	 * @param parent
	 *            the parent object - may be a virtual or group node
	 * @param child
	 *            the child object
	...
	public boolean checkConstraints(INode parent, INode child) {
		...
		for (int i = 0; i < constraints.length; i++) {
			...
			if (propname != null) {
				INode home = cnst.isGroupby ? parent : nonvirtual;
				String value = null;
	...
	/**
	 * Make sure that the child node fullfills the constraints defined by this
	 * relation by setting the appropriate properties
	 */
	public void setConstraints(INode parent, INode child) {
		...
		for (int i = 0; i < constraints.length; i++) {
				...
				if (localProp == null) {
					throw new RuntimeException(
							"Error: column "
									...
									+ Relation.this);
				} else if (foreignIsPrimary
						&& child.getState() == INode.TRANSIENT) {
					throw new RuntimeException(propName
							+ " set to transient object, "
							+ "can't derive persistent ID for " + localProp);
				} else {
					String value = foreignIsPrimary ? child.getID() : child
							.getDbMapping().getStorageTypeName();
			...
			if (crel != null) {
				...
				if (cnst.localKeyIsPrimary(home.getDbMapping())) {
					// only set node if property in child object is defined as
					// reference.
					if (crel.reftype == REFERENCE) {
						INode currentValue = child.getNode(crel.propName);

						// we set the backwards reference iff the reference is
						// currently unset, if
						// is set to a transient object, or if the new target is
						// not transient. This
						// prevents us from overwriting a persistent refererence
						// with a transient one,
						// which would most probably not be what we want.
						if ((currentValue == null)
								|| ((currentValue != home) && ((currentValue
										.getState() == INode.TRANSIENT) || (home
										.getState() != INode.TRANSIENT))))
							try {
								child.setNode(crel.propName, home);
							} catch (Exception ignore) {
								// in some cases, getNonVirtualParent() doesn't
								// work
								// correctly for transient nodes, so this may
								// fail.
							}
					} else if (crel.reftype == PRIMITIVE) {
						if (home.getState() == INode.TRANSIENT) {
							throw new RuntimeException(
									"Object is transient, can't derive persistent ID for "
											+ crel);
						}
					...
					if (cnst.localKeyIsPrototype()) {
						child.setString(crel.propName, home.getDbMapping()
								.getStorageTypeName());
					} else {
						...
						if (prop != null) {
							...
							prop = child.getProperty(cnst.foreignProperty(child
									.getDbMapping()));
							if (prop != null) {
	...
	public Map<String, String> getKeyParts(INode home) {
		...
		for (int i = 0; i < constraints.length; i++) {
			...
			if (cnst.localKeyIsPrimary(ownType)) {
				...
				map.put(cnst.foreignKey, home.getDbMapping()
						.getStorageTypeName());
			} else {
	...
	/**
	 * The Constraint class represents a part of the where clause in the query
	 * used to establish a relation between database mapped objects.
	 */
	class Constraint {
		String localKey;
		String foreignKey;
		...
		public void addToQuery(StringBuffer q, INode home, INode nonvirtual,
				DbMapping otherDbm) throws SQLException, ClassNotFoundException {
			String local;
		...
		public boolean foreignKeyIsPrimary() {
			return (foreignKey == null) || "$id".equalsIgnoreCase(foreignKey)
