...
/**
 * This is the internal representation of a database key. It is constructed from
 * the logical table (type) name and the object's primary key within the table.
 * Currently only single keys are supported.
 */
public final class DbKey implements Key, Serializable {
	...
	public int hashCode() {
		if (hashcode == 0) {
			hashcode = (storageName == null) ? (17 + (37 * id.hashCode()))
					: (17 + (37 * storageName.hashCode()) + (+37 * id
							.hashCode()));
		}
	...
	public String toString() {
		return (storageName == null) ? ("[" + id + "]") : (storageName + "["
				+ id + "]");
	}
