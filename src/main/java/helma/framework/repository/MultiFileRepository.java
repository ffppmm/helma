...
/**
 * Repository implementation that provides all of its subdirectories as
 * top-level FileRepositories
 * 
...
public class MultiFileRepository extends FileRepository {
	...
	/**
	 * Updates the content cache of the repository. We override this to create
	 * child repositories that act as top-level script repositories rather than
	 * prototype repositories. Zip files are handled as top-level script
	 * repositories like in FileRepository, while resources are ignored.
	 */
	public synchronized void update() {
		...
		if (directory.lastModified() != lastModified) {
			...
			for (int i = 0; i < list.length; i++) {
				// create both directories and zip files as top-level
				// repositories,
	...
	public String toString() {
		return new StringBuffer("MultiFileRepository[").append(name)
				.append("]").toString();
	}
}
