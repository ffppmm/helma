...
public final class ZipRepository extends AbstractRepository {
	...
	private ZipRepository(File file, Repository parent, ZipEntry zipentry) {
		...
		if (zipentry == null) {
			...
			name = new StringBuffer(parent.getName()).append('/')
	...
	public synchronized void update() {
		if (file.lastModified() != lastModified || repositories == null
				|| resources == null) {
			...
			try {
				...
				while (en.hasMoreElements()) {
					...
					if (!eName.regionMatches(0, entryPath, 0,
							entryPath.length())) {
					...
					if (depth > 0 && !shortName.equals(entrypath[depth - 1])) {
						// catch case where our name is Foo and other's is
						// FooBar
						continue;
					}

					// create new repositories and resources for all entries
					// with a
					// path depth of this.depth + 1
					if (entrypath.length == depth + 1 && !entry.isDirectory()) {
						// create a new child resource
						ZipResource resource = new ZipResource(entry.getName(),
								this);
						...
						if (!newRepositories.containsKey(entrypath[depth])) {
							...
							ZipRepository rep = new ZipRepository(file, this,
									child);
				...
				repositories = (Repository[]) newRepositories.values().toArray(
						new Repository[newRepositories.size()]);
	...
	public void create() {
		...
		throw new UnsupportedOperationException(
				"create() not implemented for ZipRepository");
	}

	/**
	 * Checks wether the repository is to be considered a top-level repository
	 * from a scripting point of view. For example, a zip file within a file
	 * repository is not a root repository from a physical point of view, but
	 * from the scripting point of view it is.
	 * 
	 * @return true if the repository is to be considered a top-level script
	 *         repository
	 */
	...
	public String toString() {
		return new StringBuffer("ZipRepository[").append(name).append("]")
				.toString();
	}
