...
public class ApplicationBean implements Serializable {
	...
	/**
	 * Get the app's event logger. This is a Log with the category
	 * helma.[appname].event.
	 * 
	...
	/**
	 * Get the app logger. This is a commons-logging Log with the category
	 * <code>logname</code>.
	 * 
	...
	/**
	 * Log a DEBUG message to the log defined by logname if debug is set to true
	 * in app.properties.
	 * 
	 * @param logname
	 *            the name (category) of the log
	...
	/**
	 * Add a repository to the app's repository list. The .zip extension is
	 * automatically added, if the original library path does not point to an
	...
	public synchronized void addRepository(Object obj) {
		...
		Repository parent = current == null ? null : current.getRepository()
				.getRootRepository();
		Repository rep;
		if (obj instanceof String) {
			...
			if (file.isDirectory()) {
				...
				throw new RuntimeException(
						"Unsupported file type in addRepository: " + file);
			}
		} else if (obj instanceof Repository) {
			rep = (Repository) obj;
		} else {
			throw new RuntimeException("Invalid argument to addRepository: "
					+ obj);
		}
	...
	/**
	 * Helper method to resolve a repository path. Returns null if no file is
	 * found.
	 * 
	 * @param parent
	 *            the parent file
	...
	/**
	 * Register a user with the given name and password using the database
	 * mapping of the User prototype
	 * 
	 * @param username
	 *            the user name
	 * @param password
	 ...
	 *            the function name
	 * @param year
	 *            comma separated list of years, or *
	 * @param month
	 *            comma separated list of months, or *
	 * @param day
	 *            comma separated list of days, or *
	 * @param weekday
	 *            comma separated list of weekdays, or *
	 * @param hour
	 *            comma separated list of hours, or *
	 * @param minute
	 *            comma separated list of minutes, or *
	 */
	public void addCronJob(String functionName, String year, String month,
			String day, String weekday, String hour, String minute) {
		CronJob job = CronJob.newJob(functionName, year, month, day, weekday,
				hour, minute);
	...
	/**
	 * Returns the app's data node used to share data between the app's
	 * evaluators
	 * 
	...
	public Map<String, Object> getSkinfiles() {
		...
		for (Iterator<Prototype> it = app.getPrototypes().iterator(); it
				.hasNext();) {
	...
	/**
	 * Return the absolute application directory (appdir property in
	 * apps.properties file)
	 * 
	...
	/**
	 * Trigger a synchronous Helma invocation with a default timeout of 30
	 * seconds.
	 * 
	 * @param thisObject
	 ...
	 *            an array of arguments
	 * @param timeout
	 *            the timeout in milliseconds. After waiting this long, we will
	 *            try to interrupt the invocation
	 * @return the value returned by the function
	 * @throws Exception
	 *             exception thrown by the function
	 */
	...
	/**
	 * Trigger an asynchronous Helma invocation. This method returns immedately
	 * with an object that allows to track the result of the function invocation
	 ...
	 * <li>waitForResult() - wait indefinitely until invocation terminates and
	 * return the result</li>
	 * <li>waitForResult(t) - wait for the specified number of milliseconds for
	 * invocation to terminate and return the result</li>
	 * </ul>
	 * 
	 * @param thisObject
	 *            the object to invoke the function on, or null for global
	 ...
	 *            the function or function name to invoke
	 * @param args
	 *            an array of arguments this long, we will try to interrupt the
	 *            invocation
	 * @return an object with the properties described above
	 */
	public FutureResult invokeAsync(Object thisObject, final Object function,
			final Object[] args) {
	...
	class AsyncInvoker extends Thread implements FutureResult {
		...
		private AsyncInvoker(Object thisObj, Object func, Object[] args,
				long timeout) {
			thisObject = thisObj;
			function = func;
			this.args = args;
		...
		public void run() {
			...
			try {
				...
				setResult(reval.invokeInternal(thisObject, function, args,
						timeout));
			} catch (Exception x) {
				setException(x);
			} finally {
		...
		public String toString() {
			return new StringBuffer("AsyncInvokeThread{running: ")
					.append(running).append(", result: ").append(result)
