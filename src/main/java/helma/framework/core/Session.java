...
/**
 * This represents a session currently using the Hop application. This includes
 * anybody who happens to request a page from this application. Depending on
 * whether the user is logged in or not, the session holds a persistent user
 * node.
 */
public class Session implements Serializable {
	...
	// used to remember messages to the user between requests, mainly between
	// redirects.
	...
	public void logout() {
		if (userHandle != null) {
			try {
				// Invoke User.onLogout() iff this is a transactor request with
				// a request
				// evaluator already associated (i.e., if this is called from an
				// app/script).
				// Otherwise, we assume being called from the scheduler thread,
				// which takes
				...
				if (reval != null) {
					...
					if (userNode != null)
						reval.invokeDirectFunction(userNode, "onLogout",
								new Object[] { sessionId });
				}
			} catch (Exception x) {
				// errors should already be logged by request evaluator, but you
				// never know
	...
	/**
	 * Called at the beginning of a request to let the session know it's being
	 * used.
	 */
	public void touch() {
	...
	/**
	 * Get the persistent user id of a registered user. This is usually the user
	 * name, or null if the user is not logged in.
	 */
	...
	/**
	 * Set the user and debug messages over from a previous response. This is
	 * used for redirects, where messages can't be displayed immediately.
	 * 
	 * @param res
	 ...
	 * This is used for redirects, where messages can't be displayed
	 * immediately.
	 * 
	 * @param res
	...
	/**
	 * Return the message that is to be displayed upon the next request within
	 * this session.
	 * 
	...
	/**
	 * Set a message to be displayed to this session's user. This can be used to
	 * save a message over to the next request when the current request can't be
	 * used to display a user visible message.
	 * 
	 * @param msg
	 *            the message
	 */
	...
	/**
	 * Return the debug buffer that is to be displayed upon the next request
	 * within this session.
	 * 
	...
	/**
	 * Set the debug buffer to be displayed to this session's user. This can be
	 * used to save the debug buffer over to the next request when the current
	 * request can't be used to display a user visible message.
	 * 
	 * @param buffer
	 *            the buffer
	 */
	...
	protected void pruneUploads() {
		if (uploads == null || uploads.isEmpty())
			return;
		for (Iterator<UploadStatus> it = uploads.values().iterator(); it
				.hasNext();) {
