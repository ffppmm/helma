...
/**
 * This represents a Helma skin, i.e. a template created from containing Macro
 * tags that will be dynamically evaluated.. It uses the request path array from
 * the RequestEvaluator object to resolve Macro handlers by type name.
 */
public final class Skin {
	...
	/**
	 * Create a skin without any restrictions on which macros are allowed to be
	 * called from it
	 */
	...
	/**
	 * Create a skin with a sandbox which contains the names of macros allowed
	 * to be called
	 */
	...
	public static Skin getSkin(Resource res, Application app)
			throws IOException {
	...
	/**
	 * Check if this skin has a main skin, as opposed to consisting just of
	 * subskins
	 * 
	...
	public String[] getSubskinNames() {
		return subskins == null ? new String[0] : (String[]) subskins.keySet()
	 ...
	 */
	public String renderAsString(RequestEvaluator reval, Object thisObject,
			Object paramObject) throws RedirectException,
	 ...
	 * Render this skin
	 */
	public void render(RequestEvaluator reval, Object thisObject,
			Object paramObject) throws RedirectException,
			UnsupportedEncodingException {
		...
		try {
			...
			RenderContext cx = new RenderContext(reval, thisObject,
					handlerCache);
	...
	/**
	 * Check if a certain macro is present in this skin. The macro name is in
	 * handler.name notation
	 */
	...
	/**
	 * Adds a macro to the list of allowed macros. The macro is in handler.name
	 * notation.
	 */
	...
	public class Macro {
		 ...
		 * 
		 * @param start
		 *            the start of the macro within the skin source
		 * @param macroOffset
		 *            offset of the macro content from the start index
		 */
			...
			if (".extends".equals(name)) {
				...
				if (positionalParams == null || positionalParams.size() < 1
					...
					throw new RuntimeException(
							".extends requires an unnamed string parameter");
				}
		...
		private int parse(int macroOffset, boolean lenient) {
			...
			loop: for (i = start + macroOffset; i < length - 1; i++) {
				...
				case '/':
					...
					if (state == PARSE_MACRONAME && "//".equals(b.toString())) {
						isCommentMacro = true;
						// just continue parsing the macro as this is the only
						// way
				...
				case '\'':
					...
					if (!escape && state == PARSE_PARAM) {
						if (quotechar == source[i]) {
							if (source[i + 1] != '%'
									&& !Character.isWhitespace(source[i + 1])
									&& !lenient) {
								// closing quotes and next character is not
								// space or end tag -
				...
				case '=':
					...
					if (!escape && quotechar == '\u0000'
							&& state == PARSE_PARAM && lastParamName == null) {
				...
				if (i == length - 2 && !lenient
					...
					// macro tag is not properly terminated, switch to lenient
					// mode
					reset();
					return parse(macroOffset, true);
				}
		...
		private void addParameter(String name, Object value) {
			...
			if ("prefix".equals(name)) {
				...
				if ("html".equals(value)) {
					...
					app.logEvent("Unrecognized encoding in skin macro: "
							+ value);
				}
		...
		private Object invokeAsMacro(RenderContext cx,
				StandardParams stdParams, boolean asObject) throws Exception {
			...
			if ((sandbox != null) && !sandbox.contains(name)) {
				throw new MacroException("Macro not allowed in sandbox: "
						+ name);
			}
			...
			if (handlerType == HANDLER_GLOBAL || handler != null) {
				...
				String funcName = resolveFunctionName(handler, propName
						+ "_macro", engine);
				...
				if (funcName != null) {
					...
					// get reference to rendered named params for after
					// invocation
					Map<?, ?> params = (Map<?, ?>) arguments[0];
					value = cx.reval.invokeDirectFunction(handler, funcName,
							arguments);

					// update StandardParams to override defaults in case the
					// macro changed anything
					if (stdParams != null)
						stdParams.readFrom(params);

					// if macro has a filter chain and didn't return anything,
					// use output
					// as filter argument.
					...
					// display error message unless onUnhandledMacro is defined
					// or silent failmode is on
					if (!engine.hasProperty(handler, propName)) {
						if (engine.hasFunction(handler, "onUnhandledMacro",
								false)) {
							Object[] arguments = prepareArguments(1, cx);
							arguments[0] = propName;
							value = cx.reval.invokeDirectFunction(handler,
									"onUnhandledMacro", arguments);
							// if macro has a filter chain and didn't return
							// anything, use output
							// as filter argument.
							if (asObject && value == null
									&& buffer.length() > bufLength) {
						...
						} else if (standardParams.verboseFailmode(handler,
								engine)) {
		...
		/**
		 * Render this macro as nested macro, only converting to string if
		 * necessary.
		 */
		 ...
		 */
		void render(RenderContext cx) throws RedirectException,
				UnsupportedEncodingException {
			StringBuffer buffer = cx.reval.getResponse().getBuffer();
			// remember length of response buffer before calling macro
			int bufLength = buffer.length();
			try {
				...
				if (buffer.length() == bufLength) {
					// If the macro function didn't write anything to the
					// response itself,
					...
					// Append macro return value even if it wrote something to
					// the response,
					// but don't render default value in case it returned
					// nothing.
		...
		private Object[] prepareArguments(int offset, RenderContext cx)
				throws Exception {
			int nPosArgs = (positionalParams == null) ? 0 : positionalParams
					.size();
			...
			if (namedParams == null) {
				...
				for (Iterator<Map.Entry<String, Object>> it = namedParams
				...
				// pass a clone/copy of the parameter map so if the script
				// changes it,
		...
		private Object resolvePath(Object handler, RequestEvaluator reval)
			...
			for (int i = 1; i < path.length - 1; i++) {
				...
				Object next = reval.invokeDirectFunction(handler,
						"getMacroHandler", arguments);
				if (next != null) {
					handler = next;
				} else if (!reval.scriptingEngine.isTypedObject(handler)) {
					handler = reval.scriptingEngine.getProperty(handler,
							path[i]);
					if (handler == null) {
						return null;
					}
			...
			if (handlerType == HANDLER_GLOBAL) {
				...
				if (macroPath == null || macroPath.length == 0) {
					...
					for (int i = 0; i < macroPath.length; i++) {
						...
						String funcName = path == null || path.length() == 0 ? functionName
								: path + "." + functionName;
		...
		void writeResponse(Object value, RequestEvaluator reval,
				StandardParams stdParams, boolean useDefault) throws Exception {
			String text;
			...
			if ((text != null) && (text.length() > 0)) {
				// only write prefix/suffix if value is not null, if we write
				// the default
				// value provided by the macro tag, we assume it's already
				// complete
	...
	class StandardParams {
		...
		boolean containsMacros() {
			return !(prefix instanceof String) || !(suffix instanceof String)
		...
		boolean verboseFailmode(Object handler, ScriptingEngine engine) {
			return (failmode == FAIL_VERBOSE)
					|| (failmode == FAIL_DEFAULT && (handler == null || engine
							.isTypedObject(handler)));
		}
	...
	class RenderContext {
		...
		RenderContext(RequestEvaluator reval, Object thisObject,
		...
		private Object resolveHandler(String handlerName, int handlerType) {
			...
			if (thisObject != null) {
				...
				// was called with this object - check this-object for matching
				// prototype
