...
public final class SkinManager implements FilenameFilter {
	...
	public Skin getSkin(Prototype prototype, String skinname, String subskin,
			Object[] skinpath) throws IOException {
		Prototype proto = prototype;

		// Loop over prototype chain and check skinpath and prototype skin
		// resources
		while (proto != null) {
			Skin skin;
			if (skinpath != null) {
				for (int i = 0; i < skinpath.length; i++) {
					...
					if (skin != null) {
						...
						if (baseskin != null && !baseskin.equals(skinname)) {
							return getSkin(prototype, baseskin, subskin,
									skinpath);
						}
					}
				}
			...
			// the next step is to look if it is defined as skin file in the
			...
			// still not found. See if there is a parent prototype which might
			// define the skin.
	...
	private Skin getSkinInPath(Object skinset, String prototype, String skinname)
			throws IOException {
		...
		if (skinset instanceof INode) {
			...
			b.append(File.separatorChar).append(prototype)
					.append(File.separatorChar).append(skinname)
					.append(skinExtension);
		...
		// the sequence is prototype.skin-from-db, prototype.skin-from-file,
		// parent.from-db, parent.from-file etc.
		return null;
	}
