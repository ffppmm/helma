 ...
 * specified by the application has passed. In the latter case, the evaluator
 * thread is killed and an error message is returned.
 */
public final class RequestEvaluator implements Runnable {
	...
	protected synchronized void initScriptingEngine() {
		if (scriptingEngine == null) {
			...
			try {
				...
				Class<?> clazz = app.getClassLoader()
     ...
     */
	public void run() {
		// first, set a local variable to the current transactor thread so we
		// know
		...
		try {
			...
			while (localThread == thread) {
				...
				// Get req and res into local variables to avoid memory caching
				// problems
				...
				String functionName = function instanceof String ? (String) function
						: null;
				...
				while (!done && localThread == thread) {
					...
					try {
						...
						// avoid going into transaction if called function
						// doesn't exist.
						// this only works for the (common) case that method is
						// a plain
						// method name, not an obj.method path
						if (reqtype == INTERNAL) {
							// if object is an instance of NodeHandle, get the
							// node object itself.
							if (thisObject instanceof NodeHandle) {
								thisObject = ((NodeHandle) thisObject)
										.getNode(app.nmgr.safe);
							...
							if (functionName != null
									&& !scriptingEngine.hasFunction(thisObject,
											functionName, true)) {
								app.logEvent(missingFunctionMessage(thisObject,
										functionName));
								done = true;
								reqtype = NONE;
								break;
							...
							// only internal requests may pass a function
							// instead of a function name
							throw new IllegalStateException(
									"No function name in non-internal request ");
						}

						// Update transaction name in case we're processing an
						// error
						if (error != null) {
							txname = "error:" + txname;
						}
						...
						case HTTP:
							...
							// catch redirect in path resolution or script
							// execution
							try {
								// catch object not found in path resolution
								try {
									if (error != null) {
										// there was an error in the previous
										// loop, call error handler
										currentElement = root;
										res.setStatus(500);

										// do not reset the requestPath so error
										// handler can use the original one
										// get error handler action
										String errorAction = app.props
												.getProperty("error", "error");
										...
										for (int i = 0; i < ntokens; i++) {
											...
											// if we're at the last element of
											// the path,
											// try to interpret it as action
											// name.
											if (i == (ntokens - 1)
													&& !req.getPath().endsWith(
															"/")) {
												action = getAction(
														currentElement,
														pathItems[i], req);
											...
											if (action == null) {
												currentElement = getChildElement(
														currentElement,
														pathItems[i]);

												// add object to request path if
												// suitable
												if (currentElement != null) {
									...
									if (error != null) {
										...
										// we already have an error and the
										// error template wasn't found,
										// display it instead of notfound
										// message
										throw new RuntimeException();
									}

									// The path could not be resolved. Check if
									// there is a "not found" action
									// specified in the property file.
									res.setStatus(404);

									String notFoundAction = app.props
											.getProperty("notfound", "notfound");
								...
								// register path objects with their prototype
								// names in
								// res.handlers
								Map<String, Object> macroHandlers = res
										.getMacroHandlers();
								...
								for (int i = 0; i < l; i++) {
									...
									// immediately register objects with their
									// direct prototype name
									if (protos[i] != null) {
										macroHandlers.put(protos[i].getName(),
												obj);
										macroHandlers.put(
												protos[i].getLowerCaseName(),
												obj);
									}
								}

								// in a second pass, we register path objects
								// with their indirect
								// (i.e. parent prototype) names, starting at
								// the end and only
								// if the name isn't occupied yet.
								for (int i = l - 1; i >= 0; i--) {
									if (protos[i] != null) {
										protos[i].registerParents(
												macroHandlers,
												requestPath.get(i));
									}
								}
								...
								// set the req.action property, cutting off the
								// _action suffix
								...
								Object actionProcessor = req.getActionHandler() != null ? req
								...
								if (req.isXmlRpc()) {
									...
									result = scriptingEngine.invoke(
											currentElement, actionProcessor,
											args.toArray(),
									...
									scriptingEngine.invoke(currentElement,
											actionProcessor, EMPTY_ARGS,
											ScriptingEngine.ARGS_WRAP_DEFAULT,
											false);
								}

								// try calling onResponse() function on object
								// before
								// calling the actual action
								scriptingEngine.invoke(currentElement,
										"onResponse", EMPTY_ARGS,
										ScriptingEngine.ARGS_WRAP_DEFAULT,
										false);

							} catch (RedirectException redirect) {
								// if there is a message set, save it on the
								// user object for the next request
								if (res.getRedirect() != null)
									session.storeResponseMessages(res);
							}

							// check if request is still valid, or if the
							// requesting thread has stopped waiting already
						...
						case EXTERNAL:
							...
							try {
								...
								if (reqtype == XMLRPC) {
									...
									String proto = app
											.getPrototypeName(currentElement);
									app.checkXmlRpcAccess(proto, functionName);
								}

								// reset skin recursion detection counter
								skinDepth = 0;
								if (!scriptingEngine.hasFunction(
										currentElement, functionName, false)) {
									throw new NotFoundException(
											missingFunctionMessage(
													currentElement,
													functionName));
								}
								result = scriptingEngine
										.invoke(currentElement,
												functionName,
												args,
												ScriptingEngine.ARGS_WRAP_XMLRPC,
												false);
								...
								// If the transactor thread has been killed by
								// the invoker thread we don't have to
						...
						// check if request is still valid, or if the requesting
						// thread has stopped waiting already
						if (localThread != thread) {
							return;
						}
						abortTransaction();
						...
						if (++tries < 8) {
							...
							try {
								...
								int base = 800 * tries;
								Thread.sleep((long) (base + (Math.random()
										* base * 2)));
							} catch (InterruptedException interrupt) {
								// we got interrrupted, create minimal error
								// message
							...
							// error in error action. use traditional minimal
							// error message
						...
						// If the transactor thread has been killed by the
						// invoker thread we don't have to
						// bother for the error message, just quit.
						if (localThread != thread) {
							return;
						}
						...
						if (error == null) {
							...
							// set done to false so that the error will be
							// processed
							done = false;
							error = x;
							...
							if (req.isXmlRpc()) {
								// if it's an XML-RPC exception immediately
								// generate error response
								if (!(x instanceof Exception)) {
									// we need an exception to pass to XML-RPC
									// responder
	...
	/**
	 * Called by the transactor thread when it has successfully fulfilled a
	 * request.
	 * 
	 * @throws Exception
	 *             transaction couldn't be committed
	 */
	...
	/**
	 * Called by the transactor thread when the request didn't terminate
	 * successfully.
	 */
	...
	private synchronized void startTransactor() {
		...
		if ((thread == null) || !thread.isAlive()) {
			...
			thread = new Thread(app.threadgroup, this, app.getName() + "-"
					+ (++threadId));
	...
	synchronized void notifyAndWait() {
		...
		// if localrtx != rtx, the current thread has been aborted and there's
		// no need to notify
		if (localThread != thread) {
		...
		if ((reqtype == NONE) && (thread == localThread)) {
			// comment this in to release not just the thread, but also the
			// scripting engine.
			// currently we don't do this because of the risk of memory leaks
			// (objects from
	 ...
	 */
	public synchronized ResponseTrans invokeHttp(RequestTrans req,
	...
	/**
	 * This checks if the Evaluator is already executing an equal request. If
	 * so, attach to it and wait for it to complete. Otherwise return null, so
	 * the application knows it has to run the request.
	 */
	...
	/*
	 * TODO invokeXmlRpc(), invokeExternal() and invokeInternal() are basically
	 * the same and should be unified
	 */

	/**
	 * Invoke a function for an XML-RPC request. The function is dispatched in a
	 * new thread and waits for it to finish.
	 * 
	 * @param functionName
	...
	public synchronized Object invokeXmlRpc(String functionName, Object[] args)
		...
		// reset res for garbage collection (res.data may hold reference to
		// evaluator)
		res = null;
	...
	/**
	 * Invoke a function for an external request. The function is dispatched in
	 * a new thread and waits for it to finish.
	 * 
	 * @param functionName
	 *            the name of the function to invoke
	 * @param args
	...
	/**
	 * Invoke a function internally and directly, using the thread we're running
	 * on.
	 * 
	 * @param obj
	 *            the object to invoke the function on
	 * @param function
	 *            the function or name of the function to invoke
	 * @param args
	 *            the arguments
	 * @return the result returned by the invocation
	 * @throws Exception
	 *             any exception thrown by the invocation
	 */
	public Object invokeDirectFunction(Object obj, Object function,
	 ...
	 *            the arguments
	 * @param timeout
	 *            the time in milliseconds to wait for the function to return,
	 ...
	 * 
	 * @param function
	 *            the function name or object
	 * @param reqtype
	 *            the request type
	 * @param reqtypeName
	 *            the request type name
	 */
	private synchronized void initObjects(Object function, int reqtype,
			String reqtypeName) {
	 ...
	 * @throws ScriptingException
	 */
	private Object getChildElement(Object obj, String name)
			throws ScriptingException {
		if (scriptingEngine.hasFunction(obj, "getChildElement", false)) {
			return scriptingEngine.invoke(obj, "getChildElement",
					new Object[] { name }, ScriptingEngine.ARGS_WRAP_DEFAULT,
					false);
		}
	...
	/**
	 * Check if an action with a given name is defined for a scripted object. If
	 * it is, return the action's function name. Otherwise, return null.
	 */
	public String getAction(Object obj, String action, RequestTrans req) {
		...
		// if no method specified or "ordinary" request try action without
		// method
