...
public final class TypeManager {
	final static String[] standardTypes = { "User", "Global", "Root",
			"HopObject" };
	...
	public TypeManager(Application app, String ignore) {
		...
		if (helmajar == null) {
			...
			URL[] urls = ((URLClassLoader) TypeManager.class.getClassLoader())
					.getURLs();
		...
		if (helmajar == null) {
			// throw new
			// RuntimeException("helma.jar not found in embedding classpath");
	...
	/**
	 * Run through application's prototype directories and create prototypes,
	 * but don't compile or evaluate any scripts.
	 */
	...
	/**
	 * Run through application's prototype directories and check if anything has
	 * been updated. If so, update prototypes and scripts.
	 */
	...
	protected synchronized void checkRepository(Repository repository,
	...
	/**
	 * Run through application's prototype sources and check if there are any
	...
	private synchronized void checkRepositories() throws IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			long lastScan = lastRepoScan.containsKey(repository) ? ((Long) lastRepoScan
					.get(repository)).longValue() : 0;
			if (repository.lastModified() != lastScan) {
				lastRepoScan.put(repository,
						new Long(repository.lastModified()));
				checkRepository(repository, false);
			}
		}

		boolean debug = "true".equalsIgnoreCase(app
				.getProperty("helma.debugTypeManager"));
		if (debug) {
			System.err.println("Starting CHECK loop in "
		...
		for (Iterator<Prototype> i = prototypes.values().iterator(); i
				.hasNext();) {
			Prototype proto = (Prototype) i.next();

			if (debug) {
				System.err.println("CHECK: " + proto.getName() + " in "
						+ Thread.currentThread());
			}
	...
	/**
	 * Returns the last time any resource in this app was modified. This can be
	 * used to find out quickly if any file has changed.
	 */
	 ...
	 */
	public synchronized Prototype createPrototype(String typename,
			Repository repository, Map<?, ?> typeProps) {
		if ("true".equalsIgnoreCase(app.getProperty("helma.debugTypeManager"))) {
			System.err.println("CREATE: " + typename + " from " + repository
