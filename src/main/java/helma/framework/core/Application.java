...
import helma.orm.db.NodeManager;
...
/**
 * The central class of a Helma application. This class keeps a pool of request
 * evaluators (threads with JavaScript interpreters), waits for requests from
 * the Web server or XML-RPC port and dispatches them to the evaluators.
 */
public final class Application implements Runnable {
	...
	/**
	 * The type manager checks if anything in the application's prototype
	 * definitions has been updated prior to each evaluation.
	 */
	...
	// the set of prototype/function pairs which are allowed to be called via
	// XML-RPC
	private HashSet<String> xmlrpcAccess;

	// the name under which this app serves XML-RPC requests. Defaults to the
	// app name
	...
	/**
	 * Build an application with the given name and server instance. The app
	 * directories will be created if they don't exist already.
	 */
	public Application(String name, Server server) throws RemoteException,
			IllegalArgumentException {
		this(name, server, new Repository[0], null, null);
	}

	/**
	 * Build an application with the given name, server instance, sources and db
	 * directory.
	 */
	public Application(String name, Server server, Repository[] repositories,
			File customAppDir, File customDbDir) throws RemoteException,
			IllegalArgumentException {
		...
		if (repositories.length == 0) {
			throw new java.lang.IllegalArgumentException(
					"No sources defined for application: " + name);
		}

		this.name = name;

		this.caseInsensitive = "true".equalsIgnoreCase(server
				.getAppsProperties(name).getProperty("caseInsensitive"));
		...
		// get log names
		accessLogName = props.getProperty("accessLog", new StringBuffer(
				"helma.").append(name).append(".access").toString());
		eventLogName = props.getProperty("eventLog", new StringBuffer("helma.")
				.append(name).append(".event").toString());
		...
		if (hopHome != null) {
			parentpwfile = new CryptResource(new FileResource(new File(hopHome,
					"passwd")), null);
		}

		pwfile = new CryptResource(repositories[0].getResource("passwd"),
				parentpwfile);
	...
	/**
	 * Get the application ready to run, initializing the evaluators and type
	 * manager.
	 */
	public void init() throws DatabaseException, IllegalAccessException,
	 ...
	 */
	public void init(final String ignoreDirs) throws DatabaseException,
	...
	// We need to call initialize in a fresh thread because the calling thread
	// could
	// already be associated with a rhino context, for example when starting
	// from the
	// manage application.
	class Initializer extends Thread {
		...
		private void initInternal() throws DatabaseException,
				IllegalAccessException, InstantiationException,
			...
			if (Server.getServer() != null) {
				Vector<HelmaExtension> extensions = Server.getServer()
						.getExtensions();
			...
			if (rootMapping == null)
				throw new RuntimeException("rootPrototype does not exist: "
						+ rootPrototype);
			userMapping = getDbMapping(userPrototype);
			if (userMapping == null)
				throw new RuntimeException("userPrototype does not exist: "
						+ userPrototype);
			...
			String usernameField = (userMapping != null) ? userMapping
					.getNameField() : null;
	...
	public synchronized void stop() {
		...
		// stop evaluators
		if (allThreads != null) {
			for (Enumeration<RequestEvaluator> e = allThreads.elements(); e
					.hasMoreElements();) {
		...
		try {
			...
			if (maxThreadsProp != null) {
				try {
					...
					logEvent("Couldn't parse maxThreads property: "
							+ maxThreadsProp);
				}
			}
	...
	/**
	 * This can be used to set the maximum number of evaluators which will be
	 * allocated. If evaluators are required beyound this number, an error will
	 * be thrown.
	 */
		...
		synchronized (allThreads) {
			if (n > current) {
				...
				for (int i = 0; i < toBeDestroyed; i++) {
					try {
						RequestEvaluator re = (RequestEvaluator) freeThreads
								.pop();
	...
	public ResponseTrans execute(RequestTrans req) {
		...
		// are we responsible for releasing the evaluator and closing the
		// result?
		boolean primaryRequest = false;

		try {
			// first look if a request with same user/path/data is already being
			// executed.
			// if so, attach the request to its output instead of starting a new
			// evaluation
			...
			if (ev != null) {
				...
				if (res != null) {
					// we can only use the existing response object if the
					// response
	...
	/**
	 * Called to execute a method via XML-RPC, usually by
	 * helma.main.ApplicationManager which acts as default handler/request
	 * dispatcher.
	 */
	...
	/**
	 * Reset the application's object cache, causing all objects to be refetched
	 * from the database.
	 */
	...
	/**
	 * Set the application's root element to an arbitrary object. After this is
	 * called with a non-null object, the helma node manager will be bypassed.
	 * This function can be used to script and publish any Java object structure
	 * with Helma.
	 */
	...
	/**
	 * Return the prototype of the object to be used as this application's root
	 * object
	 */
	...
	/**
	 * Returns the node manager for this application. The node manager is the
	 * gateway to the helma.objectmodel packages, which perform the mapping of
	 * objects to relational database tables or the embedded database.
	 */
	...
	/**
	 * Returns a wrapper containing the node manager for this application. The
	 * node manager is the gateway to the helma.objectmodel packages, which
	 * perform the mapping of objects to relational database tables or the
	 * embedded database.
	 */
	...
	/**
	 * Return a transient node that is shared by all evaluators of this
	...
	/**
	 * Returns a Node representing a registered user of this application by his
	 * or her user name.
	 */
	...
	/**
	 * Return a prototype for a given node. If the node doesn't specify a
	 * prototype, return the generic hopobject prototype.
	 */
	...
	/**
	 * Return a collection containing all prototypes defined for this
	 * application
	 */
	...
	/**
	 * Programmatically define a new prototype. If a prototype with this name
	 * already exists return the existing prototype.
	 * 
	 * @param name
	 *            the prototype name
	 * @param typeProps
	 *            custom type properties map
	...
	/**
	 * Return a skin for a given object. The skin is found by determining the
	 * prototype to use for the object, then looking up the skin for the
	 * prototype.
	 */
	...
	/**
	 * Return a list of Helma nodes (HopObjects - the database object
	 * representing the user, not the session object) representing registered
	...
	public List<INode> getRegisteredUsers() {
		...
		if (list.size() == 0) {
			for (Enumeration<String> e = users.properties(); e
					.hasMoreElements();) {
	...
	public boolean loginSession(String uname, String password, Session session) {
		// Check the name/password of a user and log it in to the current
		// session
		if (uname == null) {
			return false;
		}
		...
		try {
			INode users = getUserRoot();
			PersistentNode unode = (PersistentNode) users
					.getChildElement(uname);
			if (unode == null)
				return false;
	 ...
	 * Instead, the user is authenticated against a passwd file in the
	 * application directory. This is to have some sort of authentication
	 * available *before* the application is up and running, i.e. for
	 ...
	 *             if the application's charset property is not a valid encoding
	 *             name
	 */
	...
	/**
	 * Return a path to be used in a URL pointing to the given element and
	 * action
	 * 
	 * @param elem
	 *            the object to get the URL for
	 * @param actionName
	...
	public String getNodeHref(Object elem, String actionName,
			Properties queryParams) throws UnsupportedEncodingException {
	...
	private int appendQueryParams(StringBuffer buffer, Properties params,
			String prefix, int count) throws UnsupportedEncodingException {
		for (Iterator<Entry<Object, Object>> it = params.entrySet().iterator(); it
				.hasNext();) {
			...
			if (value instanceof Map) {
				count = appendQueryParams(buffer, (Properties) value, key,
						count);
			} else {
	...
	/**
	 * This method sets the base URL of this application which will be prepended
	 * to the actual object path.
	 */
	...
	/**
	 * Returns the prototype name that Hrefs in this application should start
	 * with.
	 */
	...
	/**
	 * Get the current RequestEvaluator, or null if the calling thread is not
	...
	/**
	 * Utility function invoker for the methods below. This *must* be called by
	 * an active RequestEvaluator thread.
	 */
	...
	/**
	 * Returns the correct property name which is either case sensitive or case
	 * insensitive
	 * 
	 * @param propName
	...
	// / to script any Java object: If the object implements IPathElement (as
	// does
	// / the Node class in Helma's internal objectmodel) then the corresponding
	// / method is called in the object itself. Otherwise, a corresponding
	// script function
	...
	public String getElementName(Object obj) {
		...
		Object retval = invokeFunction(obj, "getElementName",
	...
	public Object getParentElement(Object obj) {
		...
		return invokeFunction(obj, "getParentElement",
				RequestEvaluator.EMPTY_ARGS);
	}

	/**
	 * Get the name of the prototype to be used for this object. This will
	 * determine which scripts, actions and skins can be called on it within the
	 * Helma scripting and rendering framework.
	 */
	public String getPrototypeName(Object obj) {
		...
		// If an object is not found by its direct class name, a cache entry is
		// added
		// for the class name. For negative result, the string "(unmapped)" is
		// used
		// as cache value.
		//
		// Caching is done directly in classProperties, as ResourceProperties
		// have
		// the nice effect of being purged when the underlying resource is
		// updated,
		...
		while (clazz != Object.class) {
			...
			for (int i = 0; i < classes.length; i++) {
				...
				if (protoName != null) {
					// cache the class name for the object so we run faster next
					// time
	...
	/**
	 * The run method performs periodic tasks like executing the scheduler
	 * method and kicking out expired user sessions.
	 */
	public void run() {
		...
		while (Thread.currentThread() == worker) {
			...
			try {
				...
				try {
					...
					if (sleepProp != null) {
						sleepInterval = Math.max(1000,
								Integer.parseInt(sleepProp) * 1000);
					} else {
				...
				// purge sessions
				try {
					lastSessionCleanup = sessionMgr
							.cleanupSessions(lastSessionCleanup);
				} catch (Exception x) {
		...
		synchronized (activeCronJobs) {
			for (Iterator<CronRunner> i = activeCronJobs.values().iterator(); i
					.hasNext();) {
	...
	/**
	 * Executes cron jobs for the application, which are either defined in
	 * app.properties or via app.addCronJob(). This method is called by run().
	 */
	private void executeCronJobs() {
		...
		if (!activeCronJobs.isEmpty()) {
			logEvent("Cron jobs still running from last minute: "
					+ activeCronJobs);
		}
		...
		for (Iterator<CronJob> i = jobs.iterator(); i.hasNext();) {
			...
			if (job.appliesToDate(date)) {
				...
				// if the job has a long timeout or we're already late during
				// this minute
	...
	/**
	 * Check whether a prototype is for scripting a java class, i.e. if there's
	 * an entry for it in the class.properties file.
	 */
	...
	public String getJavaClassForPrototype(String typename) {
		...
		for (Iterator<Map.Entry<Object, Object>> it = classMapping.entrySet()
				.iterator(); it.hasNext();) {
	...
	/**
	 * Return a DbSource object for a given name. A DbSource is a relational
	 * database defined in a db.properties file.
	 */
	...
	public boolean addRepository(Repository rep, Repository current) {
		if (rep != null && !repositories.contains(rep)) {
			...
			// This establishes the order of compilation between
			// FileRepositories
			...
			// no parent or parent not in app's repositories, add at end of
			// list.
	...
	/**
	 * Searches for the index of the given repository for this app. The
	 * arguement must be a root argument, or -1 will be returned.
	 * 
	 * @param rep
	...
	/**
	 * Set the code resource currently being evaluated/compiled. This is used to
	 * set the proper parent repository when a new repository is added via
	 * app.addRepository().
	 * 
	 * @param resource
	...
	private synchronized void updateProperties() {
		...
		if (props.lastModified() > lastPropertyRead) {
			...
			// if rhino debugger is enabled use higher (10 min) default request
			// timeout
			String defaultReqTimeout = "true".equalsIgnoreCase(props
					.getProperty("rhino.debug")) ? "600" : "60";
			String reqTimeout = props.getProperty("requesttimeout",
					defaultReqTimeout);
			try {
			...
			rootObjectPropertyName = props
					.getProperty("rootobjectpropertyname");
			rootObjectFunctionName = props
					.getProperty("rootobjectfunctionname");
			...
			String loggerFactory = props.getProperty("loggerFactory",
					"helma.util.Logging");
			if ("helma.util.Logging".equals(loggerFactory)) {
				logDir = props.getProperty("logdir", "log");
				if (System.getProperty("helma.logdir") == null) {
					// set up helma.logdir system property in case we're using
					// it
					// FIXME: this sets a global System property, should be
					// per-app
	...
	/**
	 * Return the XML-RPC handler name for this app. The contract is to always
	 * return the same string, even if it has been changed in the properties
	 * file during runtime, so the app gets unregistered correctly.
	 */
	public String getXmlRpcHandlerName() {
		if (xmlrpcHandlerName == null) {
			xmlrpcHandlerName = props.getProperty("xmlrpcHandlerName",
					this.name);
		}
