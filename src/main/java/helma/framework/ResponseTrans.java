...
/**
 * A Transmitter for a response to the servlet client. Objects of this class are
 * directly exposed to JavaScript as global property res.
 */
public final class ResponseTrans extends Writer implements Serializable {
	...
	/**
	 * Returns the ServletResponse instance for this ResponseTrans. Returns null
	 * for internal and XML-RPC requests.
	 */
	...
	/**
	 * This is called before a skin is rendered as string (renderSkinAsString)
	 * to redirect the output to a new string buffer.
	 * 
	 * @param buf
	...
	/**
	 * Returns the content of the current string buffer and switches back to the
	 * previos one.
	 */
	...
	public synchronized StringBuffer popBuffer() {
		if (buffer == null) {
			throw new RuntimeException(
					"Can't pop string buffer: buffer is null");
		} else if (buffers == null) {
			throw new RuntimeException(
					"Can't pop string buffer: buffer stack is empty");
		}
		// get local reference
	...
	public synchronized void write(String str) {
		if (str != null) {
			if (buffer == null) {
				buffer = new StringBuffer(Math.max(str.length() + 100,
						INITIAL_BUFFER_SIZE));
			}
			buffer.append(str);
		}
	...
	/**
	 * Appends a objct to the response unchanged. The object is first converted
	 * to a string.
	 */
	...
	public synchronized void write(char[] chars, int offset, int length) {
		if (buffer == null) {
			buffer = new StringBuffer(Math.max(length + 100,
					INITIAL_BUFFER_SIZE));
		}
	...
	/**
	 * Write object to response buffer and append a platform dependent newline
	 * sequence.
	 */
	 ...
	 * that buffer exists and its length is larger than offset. str may be null,
	...
	public void debug(Object message) {
		...
		debugBuffer
				.append("<div class=\"helma-debug-line\" style=\"background: yellow; ");
	...
	/**
	 * Replace special characters with entities, including <, > and ", thus
	 * allowing no HTML tags.
	 */
	...
	/**
	 * Encode HTML entities, but leave newlines alone. This is for the content
	 * of textarea forms.
	 */
	...
	public void redirect(String url) throws RedirectException {
		...
		redir = url == null ? null : url.replaceAll("[\r\n]", "");
	...
	public void forward(String url) throws RedirectException {
		// remove newline chars to prevent response splitting attack
		forward = url == null ? null : url.replaceAll("[\r\n]", "");
	...
	/**
	 * Allow to directly set the byte array for the response. Calling this more
	 * than once will overwrite the previous output.
	 * 
	 * @param bytes
	...
	/**
	 * Write a vanilla error report. Callers should make sure the ResponeTrans
			...
			if (!"true".equalsIgnoreCase(app.getProperty("suppressErrorPage"))) {
				...
				if (app.debug()) {
					if (throwable instanceof ScriptingException) {
						...
						writeln("<pre>" + scriptx.getScriptStackTrace()
								+ "</pre>");
						writeln("<h4>Java Stack</h4>");
						writeln("<pre>" + scriptx.getJavaStackTrace()
								+ "</pre>");
					} else {
						writeln("<h4>Java Stack</h4>");
						writeln("<pre>");
	...
	/**
	 * This has to be called after writing to this response has finished and
	 * before it is shipped back to the web server. Transforms the string buffer
	 * into a byte array for transmission.
	 */
	 ...
	 *            the charset to use if no explicit charset has been set on the
	 *            response
	 * @throws UnsupportedEncodingException
	 *             if the charset is not a valid encoding name
	 */
	public synchronized void close(String defaultCharset)
			throws UnsupportedEncodingException {
		...
		if (res != null && res.isCommitted()) {
			// response was committed using HttpServletResponse directly. We
			// need
		...
		if (response == null) {
			...
			if (buffer != null) {
				...
				// make sure this is done only once, even with more requsts
				// attached
				buffer = null;
			} else {
				response = new byte[0];
		...
		if (autoETags && etag == null && lastModified == -1 && status == 200
				&& redir == null) {
			try {
	...
	/**
	 * If we just attached to evaluation we call this instead of close because
	 * only the primary thread is responsible for closing the result
	 */
	 ...
	 * 
	 * @return true if the the response wasn't modified since the client last
	 *         saw it.
	 */
	...
	/**
	 * Set the path in which to look for skins. This may contain file locations
	 * and HopObjects.
	 * 
	 * @param arr
	 *            the skin path
	...
	/**
	 * Get the path in which to look for skins. This may contain file locations
	 * and HopObjects.
	 * 
	...
	/**
	 * Set the skin currently being rendered, returning the previously active
	 * skin.
	 * 
	 * @param skin
	 *            the new active skin
