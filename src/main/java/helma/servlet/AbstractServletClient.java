...
/**
 * This is an abstract Hop servlet adapter. This class communicates with hop
 * applications via RMI. Subclasses are either one servlet per app, or one
 * servlet that handles multiple apps
 */
public abstract class AbstractServletClient extends HttpServlet {
	...
	// soft fail on file upload errors by setting flag "helma_upload_error" in
	// RequestTrans
	// if fals, an error response is written to the client immediately without
	// entering helma
	...
	public void init(ServletConfig init) throws ServletException {
		...
		try {
			totalUploadLimit = (upstr == null) ? uploadLimit : Integer
					.parseInt(upstr);
		} catch (NumberFormatException x) {
		...
		uploadSoftfail = ("true".equalsIgnoreCase(init
				.getInitParameter("uploadSoftfail")));
	...
	/**
	 * Abstract method to get the {@link helma.framework.core.Application
	 * Applicaton} instance the servlet is talking to.
	 * 
	...
	protected void service(HttpServletRequest request,
		...
		try {
			...
			if (ServletFileUpload.isMultipartContent(reqcx)) {
				...
				UploadStatus uploadStatus = getApplication().getUploadStatus(
						reqtrans);
				try {
					uploads = parseUploads(reqcx, reqtrans, uploadStatus,
							encoding);
				} catch (Exception upx) {
					...
					if (tooLarge) {
						message = "File upload size exceeds limit of "
								+ uploadLimit + " kB";
					} else {
					...
					if (uploadSoftfail || uploadStatus != null) {
						...
						sendError(response, errorCode, "Error in file upload: "
								+ message);
						return;
					}
				}
			...
			// if the response was already written and committed by the
			// application
			...
			if (restrans.countCookies() > 0) {
				...
				for (int i = 0; i < resCookies.length; i++)
					try {
						Cookie c = resCookies[i]
								.getCookie("/", resCookieDomain);
	...
	protected void writeResponse(HttpServletRequest req,
			HttpServletResponse res, RequestTrans hopreq, ResponseTrans hopres)
			throws IOException {
	...
	void sendError(HttpServletResponse response, int code, String message)
		...
		if (!"true".equalsIgnoreCase(getApplication().getProperty(
				"suppressErrorPage"))) {
	...
	void sendRedirect(HttpServletRequest req, HttpServletResponse res,
	...
	/**
	 * Forward the request to a static file. The file must be reachable via the
	 * context's protectedStatic resource base.
	 */
	...
	private boolean checkNotModified(File file, HttpServletRequest req,
		...
		// conditional GET for embedded images and stuff, check last modified
		// date.
	...
	/**
	 * Check if the session cookie is set and valid for this request. If not,
	 * create a new one.
	 */
	private void checkSessionCookie(HttpServletRequest request,
			HttpServletResponse response, RequestTrans reqtrans, String domain) {
		...
		if (protectedSessionCookie) {
			...
			if (reqtrans.getSession() == null
					|| !reqtrans.getSession().startsWith(buffer.toString())) {
	 ...
	 *            the cookie domain
	 */
	private void createSession(HttpServletResponse response, String prefix,
			RequestTrans reqtrans, String domain) {
		Application app = getApplication();
		String id = null;
		while (id == null || app.getSession(id) != null) {
			long l = secureRandom ? random.nextLong() : random.nextLong()
					+ Runtime.getRuntime().freeMemory() ^ hashCode();
			if (l < 0)
				l = -l;
	 ...
	 * separated by comma - in that case, care is taken that only the first one
	 * is considered.
	 */
	 ...
	 * array of values.
	 */
	private static void putMapEntry(Map<String, Collection<Object>> map,
	...
	protected List<FileItem> parseUploads(ServletRequestContext reqcx,
			RequestTrans reqtrans, final UploadStatus uploadStatus,
			String encoding) throws FileUploadException,
			UnsupportedEncodingException {
		// handle file upload
		DiskFileItemFactory factory = new DiskFileItemFactory();
		FileUpload upload = new FileUpload(factory);
		// use upload limit for individual file size, but also set a limit on
		// overall size
	...
	protected void parseParameters(HttpServletRequest request,
			RequestTrans reqtrans, String encoding) throws IOException {
	 ...
	 */
	public static void parseParameters(Map<String, Collection<Object>> map,
			byte[] data, String encoding, boolean isPost)
