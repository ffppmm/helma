 ...
 * helma.extensions.demo.DemoExtensions</code> to your
 * <code>server.properties</code>. a new global object <code>demo</code> that
 * wraps helma.main.Server will be added to the scripting environment.
 */
public class DemoExtension extends HelmaExtension {
	...
	public void init(Server server) throws ConfigurationException {
		try {
			// just a demo with the server class itself (which is always there,
			// obviously)
			Class.forName("helma.main.Server");
			// if helma.main.Server not found something is wrong :P
		} catch (ClassNotFoundException e) {
			throw new ConfigurationException(
					"helma-library not present in classpath. make sure helma.jar is included. get it from http://www.helma.org/");
		}
	}
	 ...
	 *             ...
	 */
	public HashMap<String, Server> initScripting(Application app,
			ScriptingEngine engine) throws ConfigurationException {
		if (!(engine instanceof RhinoEngine)) {
			throw new ConfigurationException("scripting engine "
					+ engine.toString() + " not supported in DemoExtension");
		}

		app.logEvent("initScripting DemoExtension with " + app.getName()
				+ " and " + engine.toString());

		// initialize prototypes and global vars here
		HashMap<String, Server> globals = new HashMap<String, Server>();
