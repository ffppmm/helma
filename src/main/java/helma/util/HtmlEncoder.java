...
/**
 * This is a utility class to encode special characters and do formatting for
 * HTML output.
 */
public final class HtmlEncoder {

	// transformation table for characters 128 to 255. These actually fall into
	// two
	...
	//
	// for mor on HTML entities see
	// http://www.pemberley.com/janeinfo/latin1.html and
...
/**
	...
	public final static void encode(String str, StringBuffer ret,
		...
		// where to insert the <p> tag in case we want to create a paragraph
		// later on
		...
		// and exiting means we're at the '>', not that it's a start or close
		// tag.
		byte entering = TEXT;
		byte exiting = TEXT;
		...
		for (int i = 0; i < l; i++) {
			...
			// step one: check if this is the beginning of an HTML tag, comment
			// or
			// Helma macro.
			if (c == '<') {
				if (i < (l - 2)) {
					if (!insideMacroTag && ('%' == str.charAt(i + 1))) {
						...
						if (!insideCodeTag) {
							...
							} else if (str.regionMatches(true, i + 2,
									"doctype", 0, 7)) {
						...
						if ((j > tagStart) && (j < l)) {
							String tagName = str.substring(tagStart, j)
									.toLowerCase();
			...
			if ((triggerBreak || linebreaks > 0) && !Character.isWhitespace(c)) {
				...
				if (entering != INTERNAL && exiting != INTERNAL) {
					...
					for (int k = linebreaks - 1; k >= 0; k--) {
						if (k >= swallowBreaks && k >= _entering
								&& k >= exiting) {
							ret.append("<br />");
						}
						ret.append(newLine);
			...
			case '&':
				...
				// we didn't reach a break, so encode as entity unless inside a
				// tag
				if (insideMacroTag) {
					ret.append('&');
				} else {
			...
			case '>':
				...
				// macros and comments. Rather, we check for the matching
				// closing tag.
				if (insideComment) {
					ret.append('>');
					insideComment = !((str.charAt(i - 2) == '-') && (str
							.charAt(i - 1) == '-'));
	...
	public final static void encodeXml(String str, StringBuffer ret) {
		...
		for (int i = 0; i < l; i++) {
			...
			default:
				...
				if (c < 0x20) {
					// sort out invalid XML characters below 0x20 - all but 0x9,
					// 0xA and 0xD.
					// The trick is an adaption of
