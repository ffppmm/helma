...
public class Logger implements Log {
	...
	public String toString() {
		return new StringBuffer(getClass().getName()).append("[")
				.append(canonicalName).append(",").append(logLevel).append("]")
				.toString();
	}
	...
	protected void log(String level, Object msg, Throwable exception) {
		...
		if (entries.size() < 2000) {
			...
			entries.add(new Entry(dateCache, level, message, threadId,
					exception));
		}
	}
	...
	protected synchronized void write() {
		...
		try {
			...
			if (size > 1000) {
				...
				// is definitely wrong with this logger. Write a message to std
				// err and
	...
	class Entry {
		...
		Entry(String date, String level, String message, String threadId,
				Throwable exception) {
			this.date = date;
			this.level = level;
	...
	/**
	 * return a "quiet" version of this log that routes debug() output to
	 * trace()
	 * 
	 ...
	 * verbose output at DEBUG level (dumps whole HTTP request and response
	 * headers). You can enable that output by setting the log level to TRACE.
	 */
