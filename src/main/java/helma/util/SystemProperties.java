...
/**
 * A property dictionary that is updated from a property file each time the file
 * is modified. It is also case insensitive.
 */
public final class SystemProperties extends Properties {
	...
	private long lastcheck; // time we last checked the underlying properties
							// file
	...
	/**
	 * Construct a properties object from a file name with the given default
	 * properties (ignoring case)
	 */
	public SystemProperties(String filename, SystemProperties defaultProps) {
		// System.err.println
		// ("building sysprops with file "+filename+" and node "+node);
		super(defaultProps);
	...
	/**
	 * Private method to read file if it has been changed since the last time we
	 * did
	 */
	...
	/**
	 * Reload properties. This clears out the existing entries, loads the main
	 * properties file and then adds any additional properties there may be
	 * (usually from zip files). This is used internally by addProps() and
	 * removeProps().
	 */
	private synchronized void reload() {
		...
		if (additionalProps != null) {
			for (Iterator<Properties> i = additionalProps.values().iterator(); i
					.hasNext();)
				putAll(i.next());
		}
	...
	/**
	 * Similar to load(), but adds to the existing properties instead of
	 * discarding them.
	 */
	...
	public Object put(Object key, Object value) {
		...
		return super
				.put(ignoreCase ? key.toString().toLowerCase() : key, value);
	}
	...
	public boolean containsKey(Object key) {
		...
		return super.containsKey(ignoreCase ? key.toString().toLowerCase()
				: key);
	}
	...
	public String getProperty(String name, String defaultValue) {
		...
		return super.getProperty(
				ignoreCase ? name.toLowerCase() : name.toLowerCase(),
				defaultValue);
	}
