...
/**
 * Implementation of Jakarta Commons LogFactory that supports both simple
 * console logging and logging to files that are rotated and gzipped each night.
 * 
...
public class Logging extends LogFactory {
	...
	/**
	 * Get a logger for a file name. The log file is created in the directory
	 * specified by the "log.dir" System property. If the logname is "console" a
	 * log that writes to System.out is returned.
	 */
	 ...
	 * 
	 * @param logname
	 *            the base name for the file logger
			...
			while ((runner == this) && !isInterrupted()) {
				...
				for (int i = nloggers - 1; i >= 0; i--) {
					try {
						...
						// if log hasn't been used in the last 30 minutes, close
						// it
