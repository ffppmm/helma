...
public class FileLogger extends Logger implements Log {
	...
	/**
	 * Create a file logger. The actual file names do have numbers appended and
	 * are rotated every x bytes.
	 * 
	 * @param directory
	 *            the directory
	 * @param name
	...
	/**
	 * Open the file and get a writer to it. If the file already exists, this
	 * will return a writer that appends to an existing file if it is from
	 * today, or otherwise rotate the old log file and start a new one.
	 */
	private synchronized void openFile() {
		try {
			if (logfile.exists()
					&& (logfile.lastModified() < Logging.lastMidnight())) {
			...
			writer = new PrintWriter(new FileWriter(logfile.getAbsolutePath(),
					true), false);
	...
	/**
	 * This is called by the runner thread to to make sure we have an open
	 * writer.
	 */
	 ...
	 * Returns the renamed log file for zipping, or null if the log file
	 * couldn't be rotated.
	 * 
	...
	protected synchronized File rotateLogFile() throws IOException {
		...
		if (logfile.exists() && (logfile.length() > 0)) {
			...
			// increase counter until we find an unused log archive name,
			// checking
			// both unzipped and zipped file names
			while (archive.exists() || zipped.exists()) {
				// for the next try we append a counter
				String archidx = (ct > 999) ? Integer.toString(ct) : nformat
						.format(++ct);
