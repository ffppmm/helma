...
public class ImageGenerator {
	 ...
	 * @throws IOException
	 */
	public ImageWrapper createImage(URL url) throws MalformedURLException,
			IOException {
		Image img = read(url);
		return img != null ? new ImageWrapper(img, this) : null;
	}
	...
	/**
	 * @param input
	 *            the data of the image to create
	 * 
	 * @return the newly created image
	 * @throws IOException
	 */
	...
	public BufferedImage read(ImageInputStream stream) throws IOException {
		...
		try {
			...
			ColorConvertOp cmykToRgb = new ColorConvertOp(cmykSpace, rgbSpace,
					null);
	...
	protected void write(ImageWrapper wrapper, ImageWriter writer,
		...
		if (bi.getType() == BufferedImage.TYPE_INT_ARGB && !alpha) {
			param.setSourceBands(new int[] { 0, 1, 2 });
			DirectColorModel cm = (DirectColorModel) bi.getColorModel();
			DirectColorModel newCM = new DirectColorModel(24, cm.getRedMask(),
					cm.getGreenMask(), cm.getBlueMask());
			param.setDestinationType(new ImageTypeSpecifier(newCM, newCM
