...
public class ImageFilterOp implements BufferedImageOp {
	...
	public BufferedImage filter(BufferedImage src, BufferedImage dst) {
		...
		/*
		 * ColorModel cm = src.getColorModel(); if (cm.getPixelSize() == 8) { //
		 * byte. indexed or gray: WritableRaster raster = src.getRaster(); byte
		 * pixels[] = new byte[width]; // calculate scanline by scanline in
		 * order to safe memory. // It also seems to run faster like that for
		 * (int y = 0; y < height; y++) { raster.getDataElements(0, y, width, 1,
		 * pixels); fltr.setPixels(0, y, width, 1, cm, pixels, 0, width); } }
		 * else { // integer, use the simple rgb mode: WritableRaster raster =
		 * src.getRaster(); int pixels[] = new int[width]; // calculate scanline
		 * by scanline in order to safe memory. // It also seems to run faster
		 * like that for (int y = 0; y < height; y++) {
		 * raster.getDataElements(0, y, width, 1, pixels); fltr.setPixels(0, y,
		 * width, 1, cm, pixels, 0, width); } }
		 */
		...
		// which is supposed to be fast too. This is probably the case because
		// drawing to BufferedImages uses
	 ...
	 * unchanged rgb value in getRGB, getRed, getGreen, getBlue, getAlpha. This
	 * speeds up things for BufferedImages by at least a factor 1.5!
	 */
	...
	/**
	 * This is a dummy ImageConsumser that does nothing else than writing The
	 * resulting rows from the ImageFilter into the image If the image was not
	 * specified in the constructor, setDimensions creates it with the given
	 * dimensions.
	 */
	class BufferedImageConsumer implements ImageConsumer {
		 ...
		 * will create a default INT_ARGB image of the size defined by the
		 * filter.
		 */
		...
		public void setDimensions(int w, int h) {
			if (image == null) {
				if (compatible != null) {
					...
					image = new BufferedImage(compatible.getColorModel(),
							compatible.getRaster()
									.createCompatibleWritableRaster(w, h),
							compatible.isAlphaPremultiplied(), null);
				} else {
