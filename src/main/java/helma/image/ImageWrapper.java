...
public class ImageWrapper {
	...
	public ImageWrapper(Image image, int width, int height,
		...
		// graphics are turned off by default. getGraphics activates it if
		// necessary.
		this.graphics = null;
	}
	...
	public Graphics2D getGraphics() {
		if (graphics == null) {
			// make sure the image is a BufferedImage and then create a graphics
			// object
	...
	/**
	 * Sets the internal image and clears the stored graphics object. Any code
	 * that is changing the internal image should do it through this function to
	 * make sure getGraphcis() returns a valid graphics object the next time it
	 * is called.
	 */
	 ...
	 */
	public Object clone() {
		ImageWrapper wrapper = generator.createImage(this.width, this.height);
	...
	public void crop(int x, int y, int w, int h) {
		...
		if (image instanceof BufferedImage && x + w <= width && y + h <= height) {
			...
			// Simply create it with the cropped dimensions and draw the image
			// into it:
	 ...
	 * 
	 * @param x
	 *            the x-coordinate of the pixel specifying the background color
	 * @param y
	...
	public void trim(int x, int y, boolean trimLeft, boolean trimTop,
		...
		if (trimLeft || trimRight) {
			BufferedImage column = new BufferedImage(1, height,
					BufferedImage.TYPE_INT_ARGB);
			Graphics2D g2d = column.createGraphics();
			int pixels[] = ((DataBufferInt) column.getRaster().getDataBuffer())
					.getData();
			// make sure alpha values do not add up for each row:
			g2d.setComposite(AlphaComposite.Src);
			if (trimLeft) {
	...
	protected void resize(int w, int h, boolean smooth) {
		...
		AffineTransform at = AffineTransform.getScaleInstance((double) w
	...
	public void resize(int w, int h) {
		double factor = Math.max((double) w / width, (double) h / height);
		// If the image is scaled, used the Graphcis2D method, otherwise use
		// AWT:
		if (factor > 1f) {
			...
			// As getScaledInstance is asynchronous, the ImageWaiter is needed
			// here too:
			// Image scaled =
			// ImageWaiter.waitForImage(image.getScaledInstance(w, h,
			// Image.SCALE_AREA_AVERAGING));
			// if (scaled == null)
			// throw new RuntimeException("Image cannot be resized.");

			// This version is up to 4 times faster than getScaledInstance:
			ImageFilterOp filter = new ImageFilterOp(
	 ...
	 * 
	 * @param colors
	 *            the number of colors to use, usually <= 256.
	 * @param dither
	 *            ...
	...
	public void reduceColors(int colors, boolean dither, boolean alphaToBitmask) {
		setImage(ColorQuantizer.quantizeImage(getBufferedImage(), colors,
				dither, alphaToBitmask));
	}
	...
	public void saveAs(OutputStream out, String mimeType) throws IOException {
		generator.write(this, out, mimeType, -1f, false); // -1 means default
															// quality
	}
	...
	/**
	 * Utility method to be used by write(). Converts file name to absolute path
	 ...
	 * @throws IOException
	 *             if missing directories could not be created
	 */
